...
class Base {
	...
	public static function run() {
		if (!\T6\T6::isT6()) return;
		if (\T6\T6::inEdit()) return;
	...
	protected function optimizeGroup($group) {
		...
		foreach ($group as $url => $options) {
			...
			try {
				...
				Factory::getApplication()->getLogger()->warning(
					'Failed to get file modification time: ' . $e->getMessage(),
					['context' => 't6.optimizer', 'file' => $file]
				);
			}
			$files[] = $file;
		}
		...
		$outputpath = T6PATH_MEDIA . $this->outputpath;
		$outputurl = T6PATH_MEDIA_URI . $this->outputpath;
		...
		if (!is_file($outputpath)){
			...
			try {
				...
				// Log error but continue - directory might already have correct permissions
				Factory::getApplication()->getLogger()->warning(
					'Failed to set directory permissions: ' . $e->getMessage(),
					['context' => 't6.optimizer', 'path' => $outputpath]
				);
			}
		}
