...
use T6\Helper\Rtl;
use T6\Helper\Asset;
use T6\Helper\Metadata;
use T6\Helper\Cache as T6Cache;
...
// Joomla 6: jadev plugin not needed
...
class Template
{
	...
	public function __construct($doc)
	{
		...
		if (!$this->bs5) {
			$this->wam->useAsset('script', 't6.bootstrap.js');
		}
	...
	public static function getInstance($doc)
	{
		...
		if (!$t6) {
			...
			$previewlayout = $input->get('t6previewlayout');
			$class = 'Template';
			if ($preview) {
				$class = 'Preview';
			}
			if ($previewlayout) {
				$t6 = new PreviewLayout($doc);
			}
			elseif ($preview) {
				$t6 = new Preview($doc);
			}
	...
	public function render()
	{
		...
		if ($cachekey) {
			// Get from cache
			$layout = T6Cache::loadLayout($cachekey);
			if ($layout) {
				return $layout;
			}
		}
		...
		if (preg_match_all('/\{t6:([^\}\:]*)\:?([^\}]*)\}/', $layout, $matches)) {
			$arr = [];
		...
		// store cache
		if ($cachekey) {
			T6Cache::storeLayout($cachekey, $layout);
		}
	...
	protected function postRender($buffer)
	{
		...
		if (preg_match_all('/\{t6post:([^\}\:]*)\}/', $buffer, $matches)) {
			// replace template content
			$search = [];
			$replace = [];
			$caller = $this;
	...
	protected function renderHead()
	{
		...
		$this->doc->addScript(T6PATH_BASE_URI . '/js/base.js', ['version' => 'auto']);
	...
	protected function renderOpenGraph()
	{
		...
		if (in_array($options, array('com_content', 'com_t6pagebuilder', "com_contact")))
			return;
	...
	protected function loadGoogleFonts()
	{
		...
		$file = T6PATH_ADMIN . "/etc/googlefonts/fonts.json";
	...
	{
		$layout = 't6.layout.offcanvas';
		return LayoutHelper::render($layout, $this, T6PATH . '/html/layouts');
	}
	public function getLogoedit()
	{
		...
		elseif (!$logo && $site_name || $site_slogan) {
			...
			$return = '<div class="brand">T6 Framework</div>';
		}
		return $return;
	}
	...
	public function afterRender()
	{
		...
		// Remove empty t6 layout container
		$count = 0;
		do {
			$buffer = preg_replace('/<div[^>]*class="t6-[^>]*>\s*<\/div>/mi', '', $buffer, -1, $count);
		} while ($count > 0);
	...
	{
		$t6EditLayout = $this->doc->params->get('system_t6frontendedit', 0);
		if (!(\T6\T6::inEdit() && $t6EditLayout)) {
			$isJADevOn = PluginHelper::isEnabled('system', 'jadev');

			if ($isJADevOn) {
				ScssHelper::findScssSource('t6', 'css/template.css');
			}

			// Load template CSS file
			if ($this->doc->direction == 'rtl') {
				if ($isJADevOn) {
					ScssHelper::findScssSource('t6', 'css/rtl.css');
				}

				$cssurl = Rtl::render();
				$cssurlCssPath = T6PATH_MEDIA . '/css/rtl.css';
			...
			if (is_file(T6PATH_MEDIA . '/css/' . $this->getCustomCssFilename())) {
				//check render custom css for sublayout
				$template_style_custom_css = T6PATH_MEDIA_URI . '/css/' . $this->getCustomCssFilename();
				$templateStyleHash = md5(filemtime(T6PATH_MEDIA . '/css/' . $this->getCustomCssFilename()));
	...
	public function renderSection($data)
	{
		static $counter = 1;
		// make id
		$data['id'] = 't6-' . (!empty($data['name']) ? preg_replace('/\s/', '-', strtolower($data['name'])) : 'section-' . $counter++);
		...
		return LayoutHelper::render('t6.layout.section', ['data' => $data, 'content' => $content], T6PATH . '/html/layouts');
	}
	...
	{
		return "<div class=\"t6-col {$col['cls']}\">\n{$col['content']}\n</div>";
	}

	public function renderSingleCol($col)
	{
		return !empty($col['data']['extra_class']) && !empty($col['content']) ? "<div class=\"t6-col {$col['data']['extra_class']}\">{$col['content']}</div>" : $col['content'];
		;
	}
	...
	public function renderBlock($data)
	{
		...
		if (!$file) {
			$file = Path::findT6Layout('block/' . $data['name'] . '.html');
		}
		if ($file) {
	...
	public function renderCustomCss()
	{
		...
		$versionFile = JPATH_CACHE . '/t6asset/' . $customCssFileName . '.txt';
		...
		// to css file
		$file = T6PATH_MEDIA . '/css/' . $customCssFileName;
		$hash = md5($css);
	...
	protected function loadTypelistData()
	{
		...
		// Load global params
		\T6\Helper\TemplateStyle::loadGlobalParams($this->doc);
	}
	...
	protected function getThemeData()
	{
		...
		try {
			$vals = json_decode($cc ?? '', true, 512, JSON_THROW_ON_ERROR) ?? [];
		} catch (\Throwable $e) {
			// Log error but continue with empty array
			Factory::getApplication()->getLogger()->warning(
				'Failed to decode custom colors JSON: ' . $e->getMessage(),
				['context' => 't6.document.template', 'json' => $cc]
			);
			$vals = [];
		}
