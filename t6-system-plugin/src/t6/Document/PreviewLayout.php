...
class PreviewLayout extends Preview {
	...
	public function renderRow(&$data) {
		...
		$content .= "<div class=\"t6-preview t6-preview-section\"><span>Section: {$name}</span></div>";
		return $content;
	}
	...
	public function renderContent ($data) {
		...
		$content .= "<div class=\"t6-preview t6-preview-content t6-preview-{$type}\"><span>{$type}: {$name}</span></div>";
		return $content;
	}

	public function renderSingleCol($col) {
		$extra_class = empty($col['data']['extra_class']) ? '' : ' ' . $col['data']['extra_class'];
		return "<div class=\"t6-col {$extra_class}\">{$col['content']}</div>";
	}
