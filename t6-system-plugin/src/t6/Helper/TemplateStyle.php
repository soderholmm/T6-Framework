...
class TemplateStyle {
	...
	/**
	 * Get master template style for T6 Home System
	 */
	...
	/**
	 * Update default settings for T6 Home System
	 */
	...
	/**
	 * Get default template style and check if it is T6 Home template
	 */
	public static function getDefault($params = false) {
		...
		if ($tpl === null) {
	        ...
	        // check if it is T6 Home template
	...
	public static function loadGlobalParams ($tpl) {
		...
		$global = (array)json_decode(\T6\Helper\Path::getFileContent('etc/global.json'), true);
	...
	public static function checkCurrentT6template($id)
	{
		$t6Checked = false;
		$db = Factory::getDbo();
    $query = $db->getQuery(true);
    ...
    $tpl = $db->loadResult();
    if(file_exists(JPATH_ROOT.'/templates/'.$tpl.'/error-t6.php')){
    	$t6Checked = true;
    }
    return $t6Checked;
	}
