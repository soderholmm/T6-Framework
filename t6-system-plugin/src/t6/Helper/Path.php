...
class Path {
	public static function findInTheme($which, $getUri = false, $relative = false) {
		...
		$uri = T6PATH_LOCAL_URI . '/' . $which;
		if (is_file($path)) return $getUri ? self::getUri($uri, $relative) : $path;

		$path = T6PATH_TPL . '/' . $file;
		$uri = T6PATH_TPL_URI . '/' . $which;
		if (is_file($path)) return $getUri ? self::getUri($uri, $relative) : $path;

		$path = T6PATH_BASE . '/' . $file;
		$uri = T6PATH_BASE_URI . '/' . $which;
		if (is_file($path)) return $getUri ? self::getUri($uri, $relative) : $path;
	...
	public static function findT6Layout($layout_file) {
		return self::findInTheme('html/layouts/t6/' . $layout_file);
	}
	...
	public static function getLocalContent($path) {
		$file = defined('T6PATH_LOCAL') ? T6PATH_LOCAL . '/' . $path : $path;
		return is_file($file) ? file_get_contents($file) : '';
	}

	public static function removeLocalContent($path) {
		$file = T6PATH_LOCAL . '/' . $path;
	...
	public static function saveLocalContent($path, $value) {
		...
		if (!is_dir($dir)) {
			try {
				...
				// Log error but don't fail - directory might not be creatable due to open_basedir restrictions
				try {
					Factory::getApplication()->getLogger()->warning(
						'Could not create directory: ' . $dir . ' - ' . $e->getMessage(),
						['context' => 't6.helper.path']
					);
				} catch (\Throwable $t) {
	...
	public static function addIncludePath (&$path) {
		//JPATH_THEMES
		$template_path = str_replace('\\', '/', T6PATH_TPL);
