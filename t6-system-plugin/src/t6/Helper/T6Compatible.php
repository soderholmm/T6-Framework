...
class T6Compatible {
	...
	// custom Font actions
    public static function t6Color() {
        $template = \T6Admin\Admin::getTemplate(true);
        $temp_Params = new Registry($template->params);
        $t6Theme = Path::getFileContent('etc/theme/' . $temp_Params->get('typelist-theme') . '.json');
        if (!$t6Theme) {
            $t6Theme = Path::getFileContent('etc/theme/default.json');
        }
        try {
            $datas = json_decode($t6Theme, true, 512, JSON_THROW_ON_ERROR);
        } catch (\Throwable $e) {
            // Log error but continue with empty object
            \Joomla\CMS\Factory::getApplication()->getLogger()->warning(
                'Failed to decode theme JSON: ' . $e->getMessage(),
                ['context' => 't6.helper.t6compatible', 'file' => $t6Theme]
            );
            $datas = new \stdClass();
        }
        ...
        foreach ($group_theme_colors as $group_name => $group_data) {
            ...
            foreach ($group_fields as $fields) {
                if(in_array($fields->getAttribute('type'),array('t6color','t6brand'))){
    ...
    public static function saveThemeLayout($data)
    {
        ...
        //save to local
        $file = T6PATH_LOCAL . '/etc/theme/' .$typeName . '.json';
        $dir = dirname($file);
        if (!is_dir($dir)) Folder::create($dir);
        File::write($file, $data);
        //save to base
        $fileBase = T6PATH_TPL . '/etc/theme/' .$typeName . '.json';
    ...
    public static function getCustomColors(){
        ...
        // local custom colors
        $file = T6PATH_LOCAL . '/etc/customcolors.json';
    ...
    {
    	// backup file data
        $dst = T6PATH_TPL . "/etc/backup";
        if(!is_dir($dst)) Folder::create($dst);
        $fielArr = array('site','theme');
        foreach ($fielArr as $file) {
            $FileData = T6PATH_LOCAL . 'etc/'.$file.'/' . $temp_Params->get('typelist-'.$file) . '.json';
