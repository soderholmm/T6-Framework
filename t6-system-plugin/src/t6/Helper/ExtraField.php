...
class ExtraField
{
	...
	public static function extendForm($form, $data)
	{
		...
		if ($template) {
			...
			if (is_file($filePath)) {
				...
				if (isset($xml->t6) && isset($xml->t6->basetheme)) {
					$base = trim(strtolower($xml->t6->basetheme));
				}
			}
			...
			// define const
			if (!defined('T6PATH_BASE')) define('T6PATH_BASE', T6PATH_THEMES . '/' . $base);
			if (!defined('T6PATH_BASE_URI')) define('T6PATH_BASE_URI', T6PATH_THEMES_URI . '/' . $base);
			...
			if (is_file($extended_form)) {
				Factory::getLanguage()->load('plg_system_' . T6_PLUGIN, JPATH_ADMINISTRATOR);
		...
		// Extended by T6
		$extended = T6PATH_ADMIN . '/form/' . $form_name . '.xml';
		if (is_file($extended)) {
			Factory::getLanguage()->load('plg_system_' . T6_PLUGIN, JPATH_ADMINISTRATOR);
			if (isset($data->attribs) && is_string($data->attribs)) {
				$data->attribs = json_decode($data->attribs, true);
			...
			if (count($extras)) {
				...
				if ($form->getName() == 'com_categories.categorycom_content') {
					...
					$_xml =
						...
						<form>
							<fields name="params">
								<fieldset name="t6_extrafields_params" label="T6_EXTRA_FIELDS_GROUP_LABEL" description="T6_EXTRA_FIELDS_GROUP_DESC">
									<field name="t6_extrafields" type="list" default="" show_none="true" label="T6_EXTRA_FIELDS_LABEL" description="T6_EXTRA_FIELDS_DESC">
					...
					} elseif (!empty($fdata['catid'])) { // edit
						$catid = ($fdata['catid']);
					} elseif ($input->get('option') === 'com_falang' && $input->get('task') === 'translate.edit') {
						$article_id = $input->get('article_id');
						if ($article_id) {
							$content = Table::getInstance('Content');
							$content->load($article_id);
							$catid = $content->catid;
						}
					}
					...
					if ($catid) {
						...
						if ($params instanceof Registry) {
							$extrafile = $path . '/' . $params->get('t6_extrafields') . '.xml';
							if (is_file($extrafile)) {
								Form::addFormPath($path);
								Factory::getLanguage()->load('tpl_' . $template, JPATH_SITE);
								$form->loadFile($params->get('t6_extrafields'), false);
							}
						}
					}
				}
	...
	public static function onMenuCompareForm($form, $data)
	{
		...
		if ($formName == 'com_menus.item') {
			...
			$template = TemplateStyle::getDefault();
			$xmlFile = T6PATH_BASE . '/html/' . $component . '/' . $view . '/' . $layout . '.xml';
			if (is_file($xmlFile)) {
	...
	public static function onUserCompareForm($form, $data)
	{
		...
		Form::addFormPath(T6PATH_BASE . '/params');
		Factory::getLanguage()->load('plg_system_' . T6_PLUGIN, JPATH_ADMINISTRATOR);
		$form->loadFile('user');
	}
}
