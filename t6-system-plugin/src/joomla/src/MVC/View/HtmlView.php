...
\T6\Helper\Joomla::makeAlias(JPATH_LIBRARIES . '/src/MVC/View/HtmlView.php', 'HtmlView', '_JHtmlView');
...
class HtmlView extends _JHtmlView
{
	...
	// {
	// 	$result = \T6\Helper\J3J4::isJ3() ? $this->loadTemplateJ3($tpl) : $this->loadTemplate($tpl);
	...
	{
		if (\T6\Helper\J3J4::isJ4()) return parent::loadTemplate($tpl);
		...
		if($this->_template && (strpos(str_replace('\\', '/', $this->_template), str_replace('\\', '/', T6PATH_BASE)) !== false) && $this->_template !== preg_replace('/\.php$/', '.j3.php', \JPath::find($this->_path['template'], $filetofind))){
			$this->_template = \JPath::find($this->_path['template'], $filetofind);
		}
