...
class JFormFieldTempDetail extends FormField
{
	...
	protected function getInput()
	{
		...
		//get base font name
		$customFont = \T6\Helper\Path::getFileContent('/etc/customfonts.json');

		// inject data-value attribute
		$data['googlefont'] = $googleFont;
		$data['customfont'] = @json_decode($customFont);
		return LayoutHelper::render ('field.customfont', $data, T6PATH_ADMIN . '/layouts');
	}
	...
	public function setup(SimpleXMLElement $element, $value, $group = null)
	{
		...
		//get base font name
		$customfile =  T6PATH_LOCAL . '/etc/customfonts.json';
		...
		$doc->addScript (Uri::root() . $path . '/assets/js/t6admin.js');
	...
	public function getAllT6Template(){
		$db = Factory::getDbo();
		$input = Factory::getApplication()->input;
		$id = $input->get('id');
		...
		return LayoutHelper::render('field.currentstyle', ['data' => $tempObj, 'cid' => $id], T6PATH_ADMIN . '/layouts');
