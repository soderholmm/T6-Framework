...
class JFormFieldTypeList extends FormField
{
	...
	protected function getInput()
	{
		...
		$types = \T6\Helper\Path::files('etc/' . $type);
		if (empty($types)) $types = ['default'];

		$list = [];
		foreach ($types as $name) {
			$list[$name] = T6Admin\Action\Typelist::getStatus($type, $name);
		}
		...
		// new form
		$xmlfile = \T6\Helper\Path::findInTheme('params/typelist-' . $type . '.xml');
		$basexmlfile = T6PATH_BASE . '/' .'params/typelist-' . $type . '.xml';
		$forms = Form::getInstance('typelist-' . $type, $xmlfile);
