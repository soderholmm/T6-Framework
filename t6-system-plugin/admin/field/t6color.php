 ...
 * @since  2.5
 */
class JFormFieldT6color extends FormField
{
    /**
     * The field type.
     *
    ...
    protected function getInput()
    {
        ...
        return \JLayoutHelper::render($this->layout, $data, T6PATH_ADMIN . '/layouts');
    }
    ...
    protected function loadColors()
    {
        ...
        $t6Theme = \T6\Helper\Path::getFileContent('etc/theme/' . $temp_Params->get('typelist-theme') . '.json');
        if (!$t6Theme) {
            $t6Theme = \T6\Helper\Path::getFileContent('etc/theme/default.json');
        }

        $datas = @json_decode($t6Theme);

        // $datas = T6compatible::t6Color();
    ...
    protected function getValues()
    {
        ...
        $data = @json_decode($t6Theme);
        // $data = T6compatible::t6Color();
    ...
    protected function getCustomColors(){
        ...
        $baseUsercolors = (array) json_decode(T6\Helper\Path::getFileContent('etc/customcolors.json', false), true);
        // local custom colors
