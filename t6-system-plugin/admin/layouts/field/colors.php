...
<div class="t6-select-color">
	<div class="color-preview">
		<span class="preview-icon" data-bgcolor="<?php echo $dataVal;?>" style="background-color: <?php echo $dataVal;?>;"></span>
		<input type="text" name="<?php echo $input_name;?>" id="<?php echo $input_id;?>" value="<?php echo str_replace('color','',$value_ip);?>" data-val="<?php echo str_replace("_", " ", $data_value);?>" data-color="<?php echo $dataVal;?>" class="t6-input t6-input-color <?php echo $class;?>" aria-invalid="false" readonly="" />
		<span class="toggle-icon"><i class="fal fa-angle-down"></i></span>
	</div>
	<?php
		echo \JLayoutHelper::render('field.colorTemplate', $params, T6PATH_ADMIN . '/layouts');
	?>
</div>