...
class T6menutype {
	...
	public static function onAfterGetMenuTypeOptions(&$list,$model) 
	{
        ...
        $components = self::getAllComponents();
         $lang->load('plg_system_t6',JPATH_ADMINISTRATOR);
    ...
    public static function getAllComponents(){
        ...
        if (!Admin::isT6Template($template->template)) {
            return;
        }
        ...
        if(!defined('T6PATH_BASE_HTML')){
            define('T6PATH_BASE_HTML',T6PATH_BASE . '/html/');
        }
        foreach ($components as $component) {
            if(is_dir(T6PATH_BASE_HTML . $component->option)){
                $options[] = $component;
            }
        }
        return $options;
    ...
    public static function getTypeOptionsFromMvc($component)
    {
        ...
        if (is_dir(T6PATH_BASE_HTML . $component))
        {
            $views = Folder::folders(T6PATH_BASE_HTML . $component);
        }
        $path = '';
    ...
    public static function getTypeOptionsFromLayouts($component, $view)
    {
        ...
        $layoutNames = array();
        $lang        = Factory::getLanguage();
        $path        = T6PATH_BASE_HTML . $component.'/'.$view;
