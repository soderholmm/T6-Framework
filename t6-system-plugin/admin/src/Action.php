...
class Action
{
    public static function run()
    {
        ...
        if (!Admin::isT6Template()) {
            throw new NotAllowed(Text::_('T6_ERROR_NOT_T6_TEMPLATE'), 500);
        }

        $action = Factory::getApplication()->input->getCmd('t6do');
        $action = trim(ucfirst($action));
        if ($action) {
            $class = '\\T6Admin\\Action\\' . $action;
            if (!class_exists($class)) {
                $class = '\\T6Admin\\Action';
                $func = 'do' . $action;
            } else {
    ...
    {
        $file = \T6\Helper\Path::findInTheme('css/custom.css');
    ...
    public static function doGetblock()
    {
        ...
        $file = \T6\Helper\Path::findInTheme('block/'.$blName.'.html');
        if (!$file) {
            $file = \T6\Helper\Path::findInTheme('html/layouts/t6/block/'.$blName.'.html');
        }
        $fileLocal = \T6\Helper\Path::getLocalContent('html/layouts/t6/block/'.$blName.'.html');
        $fileBase = \T6\Helper\Path::getBaseContent('html/layouts/t6/block/'.$blName.'.html');
    ...
    public static function doSaveblock()
    {
        ...
        if ($blockData !== null) {
            $path = T6PATH_LOCAL . '/html/layouts/t6/block/';
            if (!is_dir($path)) {
                Folder::create($path);
            }
            ...
            $output = ['error'=> 'T6_SAVE_BLOCK_ERROR'];
        }
        return $output;
    }
    ...
    public static function doRemoveblock()
    {
        ...
        if ($name !== null) {
            $file = T6PATH_LOCAL . '/html/layouts/t6/block/' . $name .'.html';
            if (is_file($file)) {
                unlink($file);
            }
            $file = \T6\Helper\Path::findInTheme('html/layouts/t6/block/'.$name.'.html');
            $block = $file ? file_get_contents($file) : '';
    ...
    public static function doGetPreset()
    {
        ...
        $file = \T6\Helper\Path::findInTheme('etc/presets/' . $name . '.json');
        header("Content-Type: application/json;");
    ...
    public static function doAddAddon()
    {
        ...
        // current addon
        $assetsfile = T6PATH_LOCAL . '/etc/assets.json';
        ...
        if (!isset($assets['assets'][$name])) {
            ...
            if (!File::write($assetsfile, $assets_data)) {
                $output = ['error' => Text::_('T6_ADDONS_SAVE_ERROR')];
            }
        } else {
            $output = ['error' => Text::_('T6_ADDONS_SAVE_DUPLICATED_ERROR')];
        }
    ...
    public static function doRemoveAddon()
    {
        ...
        if (isset($assets['assets'][$name])) {
            ...
            if (!File::write($assetsfile, json_encode($assets))) {
                $output = ['error' => Text::_('T6_ADDONS_DELETE_ERROR')];
            } else {
                $output = ['ok' => 1];
            }
        } else {
            $output = ['error' => Text::_('T6_ADDONS_DELETE_NOTFOUND_ERROR')];
        }
