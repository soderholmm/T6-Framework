...
class T6Compatible
{
    ...
    public static function run($temp_data)
    {
        ...
        if(self::check()){
         		$backup = T6PATH_TPL . "/backup";
            if(!is_dir($backup)){
            	self::backupFile();
    		}
            self::T6CompatibleVersions();
        }
    }
    ...
    public static function T6CompatibleVersions()
    {
        $temp_Params = self::$temp_Params;
    ...
    public static function saveSetting($type,$data)
    {
        ...
        foreach ($group_sites as $group_name => $group_data) {
            ...
            foreach ($group_fields as $fields) {
                if(!array_key_exists($fields->getAttribute('name'), $t6Data) && $fields->getAttribute('type') != 'spacer'){
                    $initCompare = true;
                    $t6Data[$fields->getAttribute('name')] = "";
                    if($type == 'site' && !empty($t6ThemeData[$fields->getAttribute('name')])){
                        if (strpos($fields->getAttribute('name'), 'font_family') !== false) {
                            ...
                            $t6Data[$font_style] = $t6ThemeData[$font_style];
                        }
                        $t6Data[$fields->getAttribute('name')] = $t6ThemeData[$fields->getAttribute('name')];
                    }elseif($type == 'theme'){
                        $t6Data[$fields->getAttribute('name')] = $fields->getAttribute('default');
                    }
                }
            }
        }
        if($initCompare){
            if($type == 'theme'){
                $t6Data['styles_palettes'] = json_encode(self::$palettes);

                //convert page color setting 
               if(!empty($allColors[str_replace(" ", "_", $t6Data['body_bg_color'])])) $t6Data['body_bg_color'] = $allColors[str_replace(" ", "_", $t6Data['body_bg_color'])];
               if(!empty($allColors[str_replace(" ", "_", $t6Data['body_text_color'])])) $t6Data['body_text_color'] = $allColors[str_replace(" ", "_", $t6Data['body_text_color'])];
               if(!empty($allColors[str_replace(" ", "_", $t6Data['link_color'])])) $t6Data['body_link_color'] = $allColors[str_replace(" ", "_", $t6Data['link_color'])];
               if(!empty($allColors[str_replace(" ", "_", $t6Data['link_hover_color'])])) $t6Data['body_link_hover_color'] = $allColors[str_replace(" ", "_", $t6Data['link_hover_color'])];
               // unset custom_colors
               if(!empty($t6Data['custom_colors'])){
                	unset($t6Data['custom_colors']);
               }
            }
            self::saveLayout($type,$temp_Params->get('typelist-'.$type),json_encode($t6Data),true);
        }
    }
    public static function saveLayout($type,$name,$data, $local = false)
    {
        //save to base
        if($name == 'palettes'){
            $fileBase = T6PATH_TPL . '/etc/' .$name . '.json';
        }else{
            $fileBase = T6PATH_TPL . '/etc/'.$type.'/' .$name . '.json';
        }
        ...
        // update file local
        if($local){
             $fileLocal = str_replace(T6PATH_TPL, T6PATH_LOCAL, $fileBase);
    ...
    public static function comparePalettes()
    {
        ...
        // local palettes
        $file = T6PATH_LOCAL . '/etc/palettes.json';
    ...
    public static function loadAllColors()
    {
        ...
        $customcolors = (array)json_decode(\T6\Helper\Path::getFileContent('etc/customcolors.json'));
    ...
    {
        // backup file data
        $dst = T6PATH_TPL . "/backup";
        if(!is_dir($dst)) Folder::create($dst);
        $localFolder = T6PATH_LOCAL . '/etc';
        $allFile = array();
        if(is_dir($localFolder)){
            $localFile = array();
            $allFile['local'] = self::scanDirectories($localFolder,$localFile);
        }
        $baseFolder = T6PATH_TPL . '/etc';
        $basefile = array();
        $allFile['base'] = self::scanDirectories($baseFolder,$basefile);
        foreach ($allFile as $nameFolder => $folders) {
            foreach ($folders as $file) {
                if($nameFolder == 'local'){
                    $file_dst = str_replace(T6PATH_LOCAL, T6PATH_TPL . "/backup/".$nameFolder, $file);
                }else{
                    $file_dst = str_replace(T6PATH_TPL, T6PATH_TPL . "/backup/".$nameFolder, $file);
                }
