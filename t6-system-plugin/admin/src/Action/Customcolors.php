...
class Customcolors {
	public static function doSave () {
		...
		$output = ["ok" => 1];
		$userColorFile = T6PATH_LOCAL . '/etc/spectrumColor.json';
		$dir = dirname($userColorFile);
		if (!is_dir($dir)) Folder::create($dir);
		$colors = is_file($userColorFile) ? explode(";",file_get_contents($userColorFile)) : explode(";",\T6\Helper\Path::getFileContent('etc/spectrumColor.json', false));
	...
	public static function doRemove () {
		...
		if (!$name) {
			return ['error' => Text::_('T6_CUSTOM_COLOR_MISSING_PARAMS')];
		}
	    $userColorFile = T6PATH_LOCAL . '/etc/customcolors.json';
	    ...
	    if (isset($colors[$name])) {
	        ...
	        if (!File::write ($userColorFile, json_encode($colors))) {
	            $output = ['error' => Text::_('T6_CUSTOM_COLOR_DELETE_ERROR')];
	        } else {
	        	$output['status'] = 'loc';
	        ...
	        $output = ['error' => Text::_('T6_CUSTOM_COLOR_DELETE_NOTFOUND_ERROR')];
	    }
