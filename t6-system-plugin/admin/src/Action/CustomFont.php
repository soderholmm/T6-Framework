...
class CustomFont {
    ...
    public static function doAdd() {
        ...
        // current addon
        $fontsfile = T6PATH_LOCAL . '/etc/customfonts.json';
        ...
        if(empty($font['type'])) return $output = ['error' => Text::_('T6_CUSTOM_FONT_SAVE_ERROR')];
        if($font['type'] == 'css'){
            $fontCss = [];
            foreach ($font['css'] as $css) {
                ...
                if(empty($content)) return $output = ['error' => Text::_('T6_CUSTOM_FONT_SAVE_ERROR')];
                ...
                foreach ($fontsElems as $name_font => $fontsElem) {
                   ...
                   if (!isset($fonts['fonts'][$name_font])) {
                        ...
                        if (!File::write ($fontsfile, json_encode($fonts))) {
                            $output = ['error' => Text::_('T6_CUSTOM_FONT_SAVE_ERROR')];
                        }
                    }else{
                        $warning_alert = true;
                    }
            ...
            foreach ($font['font'] as $fontUrl) {
                if(strpos($fontUrl,'http') === false){
                	// local
                	$file = \T6\Helper\Path::findInTheme($fontUrl);
                	if (!$file) {
                		return $output = ['error' => Text::_('T6_CUSTOM_FONT_SAVE_ERROR')];
                	}
                    $fontUrl = \T6\Helper\Path::findInTheme($fontUrl, true);
                }
        ...
        if ($warning_alert) {
            $output = ['error' => Text::_('T6_CUSTOM_FONT_SAVE_DUPLICATED_ERROR')];
        }
    ...
    public static function doRemove() {
        ...
        if (isset($fonts['fonts'][$name])) {
            ...
            $output = ['error' => Text::_('T6_CUSTOM_FONT_DELETE_NOTFOUND_ERROR')];
        }
