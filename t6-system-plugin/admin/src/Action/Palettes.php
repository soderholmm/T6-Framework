...
class Palettes {
	...
	public static function doRemove()
	{
		...
		$dir = dirname($file);
		if (!is_dir($dir)) Folder::create($dir);
		$allColor = T6Compatible::loadColors();

		// get base palettes
		$basepalettes = (array) json_decode(\T6\Helper\Path::getFileContent('etc/palettes.json', false), true);
	...
	public static function loadColors($color_name)
	{
		...
		$t6Theme = \T6\Helper\Path::getFileContent('etc/theme/' . $temp_Params->get('t6-theme') . '.json');
		if (!$t6Theme) {
