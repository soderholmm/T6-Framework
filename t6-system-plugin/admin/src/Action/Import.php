...
class Import
{
    ...
    public static function doImporting()
    {
        ...
        foreach ($groups as $group) {
            ...
            if (is_dir($import_path . '/' .$folder_name)) {
                ...
                $file = $files[0];
                $dest_file = str_replace($block_path, T6PATH_LOCAL, $file);
        ...
        if (in_array('typelist-layout', $groups)) {
            if (is_dir($block_path . '/scss')) {
                ...
                foreach ($allCssFile as $cssFile) {
                    $dest_css = str_replace($block_path, T6PATH_LOCAL, $cssFile);
            ...
            if (is_dir($block_path . '/html')) {
                ...
                foreach ($allBlockFile as $BlockFile) {
                    $dest_block = str_replace($block_path, T6PATH_LOCAL, $BlockFile);
    ...
    public static function doTest()
    {
        self::init();
        $dataError = array();
        $dir = "/Users/kienduong/works/joom/t6_dev/tmp/install_5f856cd069ed8";
        ...
        foreach ($groups as $group) {
            ...
            if (is_dir($import_path . '/' .$folder_name)) {
                $files = glob($import_path . '/' .$folder_name . '/*.json');
                $file = $files[0];
                if (!is_dir(dirname(T6PATH_LOCAL .'/etc/' .$folder_name))) {
                    Action::mkdir_r(dirname(T6PATH_LOCAL .'/etc/' .$folder_name));
                }
                if (!File::copy($file, str_replace($import_path, T6PATH_LOCAL . '/etc/', $file))) {
                    $dataError[] = basename($file) . "has copy fail";
                }
