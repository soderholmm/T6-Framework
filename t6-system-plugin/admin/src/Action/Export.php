...
use T6Admin\Admin as Admin;
use Joomla\Filesystem\Folder;
use Joomla\Filesystem\File;
use Joomla\CMS\Language\Text;
use Joomla\CMS\Uri\Uri;
use T6Admin\Action;
use T6\Helper\Path as T6Path;
class Export
{
    ...
    protected static function init()
    {
        ...
        self::$dest = self::$tmp_path . '/t6_blank' . '_' . date('Ymdhms');
        if (!is_dir(self::$dest)) {
    ...
    public static function doExport()
    {
        ...
        foreach ($agroups as $group) {
            ...
            $conf_file = T6Path::findInTheme('etc/'.$name.'/'.self::$params['typelist-'.$name] .'.json');
            if (isset($conf_file)) {
        ...
        if(in_array('Typelist-layout', $agroups)){
            ...
            if(count($blocks)){
                foreach ($blocks as $blCss => $blFile) {
                    $scss = T6Path::findInTheme('scss/layouts/'.$tplStyle.'-'.strtolower($blCss) . '.scss');
                    if($scss) {
                     ...
                     $dir_block = dirname(self::$dest . '/html/layouts/t6/block/'.basename($blFile) );
                    if (!is_dir($dir_block)) Folder::create($dir_block);
                    File::copy($blFile,self::$dest . '/html/layouts/t6/block/'.basename($blFile) );
                }
            }
        }
        // Force the download
    ...
    public static function zipper($directory, $ignore = array(), $the_file = '', $overwrite = false)
    {
        ...
        if (is_dir($directory)) {
            ...
            throw new \Exception(Text::_('COM_T6PAGEBUILDER_EXPORT_ERROR_MESSAGE_NOT_FOUND'), 404);
            return false;
        }
        ...
        if (count($files) > 0) {
            ...
            require_once T6PATH . '/vendor/nelexa/zip/src/ZipFile.php';
    ...
    public static function findBlock(){
        self::init();
        $file_layout = T6Path::findInTheme('etc/layout/'.self::$params['typelist-layout'] .'.json');
        ...
        foreach ($sections as $section) {
            foreach ($section['contents'] as $content) {
                if($content['type'] == 'block'){
                    $block_file = T6Path::findInTheme('html/layouts/t6/block/'.$content['name'].'.html');
                    if($block_file){
