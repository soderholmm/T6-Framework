...
class Typelist {
	public static function doLoad() {
		...
		$type = $input->get('type');
		$value = \T6\Helper\Path::getFileContent(self::getPath($type, $name));
	...
	public static function doClone() {
		...
		// check exist
		if (\T6\Helper\Path::findInTheme(self::getPath($type, $newname))) {
	...
	public static function doDelete() {
		...
		// check exist
		$file = T6PATH_LOCAL . '/' . self::getPath($type, $name);
		if (!is_file($file)) {
			return ['error' => 'New name existed!'];
		}
		// delete
	...
	private static function preSave($type, $name, &$value) {
		...
		if ($type == 'site' && !empty($value['other_faviconFile'])) {
			$tpl = \T6Admin\Admin::getTemplate(true);
	...
	public static function getStatus ($type, $name) {
		...
		// check local
		$lfile = T6PATH_LOCAL . $path;
		if (is_file($lfile)) $local = true;
		// check base & template
		$tfile = T6PATH_TPL . $path;
		if (is_file($tfile)) $tpl = true;
		$bfile = T6PATH_BASE . $path;
