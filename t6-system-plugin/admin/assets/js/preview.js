(function ($) {
  ...
  var addPreviewInfo = function (url) {
    if (url.indexOf("templateStyle") == -1) {
      url += url.indexOf("?") > -1 ? "&" : "?";
      url += "t6preview=" + t6previewkey + "&templateStyle=" + tempId;
    }
    var layoutpreview = $("#t6-preview-layout").prop("checked");
    if (layoutpreview) {
      url += "&t6previewlayout=1";
    }

    return url;
  };
  var removeParamUrl = function (url, name) {
    var linkbase = url.match(/^[^\#\?]+/) ? url.match(/^[^\#\?]+/)[0] : t6_site_root_url;
  ...
  var removePreviewInfo = function (url) {
    url = removeParamUrl(url, "t6preview");
    url = removeParamUrl(url, "templateStyle");
    url = removeParamUrl(url, "t6previewlayout");
    // remove t6prevew & templateStyle params
    // store
    return url;
  };

  var getPreviewLink = function (link) {
    if (!link) link = sessionStorage.getItem("last_preview_link_" + tempId);
    if (!link) {
      // get default link
      link = t6_site_root_url + $previewiframe.data("link");
    }

    if (!link || link.indexOf(t6_site_root_url) !== 0) link = t6_site_root_url;

    return addPreviewInfo(link);
  };

  var updatePreviewLink = function (link) {
    if (!link || link.indexOf(t6_site_root_url) !== 0) return;
  ...
  var removePreviewLink = function () {
    sessionStorage.removeItem("last_preview_link_" + tempId);
  };
  ...
  var _reloadPreview = function (link) {
    ...
    if (link && link.indexOf(t6_site_root_url) !== 0) {
      link = "";
    }
  ...
  var reloadAssets = function (e) {
    ...
    $('[name^="typelist-site["]').each(function () {
      var $input = $(this),
        ...
        val = $input.val();
      if (name == "body_bg_img") val = t6_site_root_url + val;
    ...
    if (palettes) {
      for (var pname in palettes) {
        ...
        palette["class"] = "t6-palette-" + pname.toLowerCase();
  ...
  var postDraft = function (type, data, firstInit) {
    ...
    postdataTimeout[type] = setTimeout(function () {
      ...
      var url =
        location.pathname +
        "?option=com_ajax&plugin=t6&format=json&t6do=draft&id=" +
        tempId;
  ...
  var init = function () {
    ...
    t6_site_root_url = window.t6_site_root_url;
    cssTplStyle = window.cssTplStyle || "";
    cssTplPalette = window.cssTplPalette || "";
    t6previewkey = window.t6previewkey || "";
    ...
    $("#t6-preview-layout").on("change", function (e) {
      reloadPreview();
    });

    $("input.t6-layouts").on("change", function (e) {
      var $this = $(e.target);
      var typelist = $this.closest(".typelist").data("typelist");
      if (!typelist.editting) return;
        if (!typelist.firstInit && !typelist.saved) {
    ...
    $(".t6-draft-preview").on("change", function (e) {
      var $this = $(this);
