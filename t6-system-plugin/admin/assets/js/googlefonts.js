...
jQuery(document).ready(function($){
    ...
    $(document).on('click touchstart', '.btn-fonts', function(e){
        ...
        if(fontCheck) T6Admin.disGgFont('dis');
        //init google font modal
        var $bodyfontmodal = $('.t6-google-font-modal');
    ...
    $(document).on('click','li .t6-font-weight-popup', function(e) {
        e.stopPropagation();
    })
    $(document).on('click','li .jub-font-container', function(e) {
        e.stopPropagation();
        e.preventDefault();
        $('.t6-font-weight-popup').appendTo($(this));
        ...
        dataWeight = "",
        clsInputFont = "#"+$(this).closest('.t6-google-font-modal').data('input');
        ...
        fontWeightRenderPopup($('.t6-font-weight-popup'),options);
        var offSet = $(this).find('.jub-font-styles').offset();
        var offSetfixed = $(this).closest('.t6-google-content').offset();
    ...
    $('body').on('click','li.custom-font', function(e){
        var nameFont = $(this).data('name'),$fontType = 'custom',
            ...
            $classInputFont = $(this).closest('.t6-google-font-modal').data('input');
    ...
    $('#t6-font-filter').on('keyup',function() {
	    ...
	    $('.t6-font-filter').show();
	    if(!value) $('.t6-font-filter').hide();
	    if(!fontCount){
	    	$filterView.find('.t6-font-filter').get(0).innerHTML = '<span class="alert alert-success">no result</span>';

	    }else{
	    	$filterView.find('.t6-font-filter').get(0).innerHTML = '<span class="alert alert-success">you have '+fontCount+' result</span>';
	    }
        $('#custom-local').hide();
    });
    // export
    ...
    var doSave = function (typefont) {
        var url = location.pathname + '?option=com_ajax&plugin=t6&format=json&t6do=customFont&task=add&id=' + tempId;
        var typeActive = $('.add-more-custom-font').find('.btn-action.active').data('type');
        var fonts = {
            type: typeActive
        };
        if (!fonts.type) {
            var mesg = $('<div class="message alert" />').append(T6Admin.langs.addonEmptyFieldTypeWaring);
        ...
        if(fonts.type == 'css'){
            ...
            if (!fonts.css) {
                var mesg = $('<div class="message alert" />').append(T6Admin.langs.fontsEmptyFieldCssWaring);
            ...
            if (!fonts.font) {
                var mesg = $('<div class="message alert" />').append(T6Admin.langs.fontEmptyFieldFontFileWaring);
        ...
        $.post(url, {fonts:fonts}).then(function(response) {
            if (response.ok && response.fonts) {
                ...
                $('.custom-font-input').val('');
                T6Admin.Messages(T6Admin.langs.T6fontCustomAdded,'message');
            } else {
    ...
    var doRemove = function (btn) {
        ...
        if (!$fontName) return;
        T6Admin.Confirm(T6Admin.langs.T6fontCustomRemoveConfirm, function(ans){
          if (ans) {
            var url = location.pathname + '?option=com_ajax&plugin=t6&format=json&t6do=customFont&task=remove&id=' + tempId;
            $.post(url, {name: $fontName}).then(function(response) {
                if (response.ok) {
                    // remove addon
                    $btn.closest('li').remove();
                    T6Admin.Messages(T6Admin.langs.T6fontCustomRemoved,'message');
                } else {
                    alert(response.error);
                }
            })
    ...
    var fontWeightRenderPopup = function($selector,$data){
        ...
        if($div){
            $div += '<div class="btn-actions"><button  type="button" data-action="btn.cancel" class="t6-btn btn-action">Cancel</button>';
            $div += '<button type="button" data-action="btn.selected" data-name_font="'+$data.name+'" data-input_font="'+$data.clsInputFont+'" class="t6-btn btn-action btn-primary">Select font</button></div>';
        }
        $selector.html($div);

    }
    $(document).on('click','.t6-btn[data-action="btn.selected"]', function(e) {
        var dataWeight = [],nameFont = $(this).data('name_font'),class_input_font = $(this).data('input_font');
        $('.t6-form-checkbox').find('.form-check-input').each(function(){
        ...
        if(dataWeight.length){
            if(parentELfont.find('.t6-font-weight-preview').length){
                parentELfont.find('.t6-font-weight-preview').html("<span class='font-weight'>Font weight: <small>"+dataWeight.join("</small><small>")+"</small></span>");
            }else{
                parentELfont.append($("<div class='t6-font-weight-preview control-helper' data-fontname='"+nameFont+"' data-loadweights='"+dataWeight.join(",")+"' />").append("<span class='font-weight'>Font weight: <small>"+dataWeight.join("</small><small>")+"</small></span>"));
            }
        } else {
            parentELfont.find('.t6-font-weight-preview').html("");
        }
        parentELfont.find('.t6-font-weight-preview').data('fontname',nameFont);
        parentELfont.find('.t6-font-weight-preview').data('loadweights',JSON.stringify(dataWeight));
        $(class_input_font).trigger('change');
        $('body').removeClass('t6-modal-open');
        $('.font-active').removeClass('font-active');
         $(".t6-font-weight-popup").hide();
        $('.themeConfigModal').children().not('.t6-message-container').hide();

    });
    $(document).on('click','.t6-btn[data-action="btn.cancel"]', function(e) {
        $(".t6-font-weight-popup").hide();
    });
    $(document).on('click', '.t6-fonts-manager', function(e) {
         $(".t6-font-weight-popup").hide();
         $('li.jub-font.font-focus').removeClass('font-focus');
    });
});
