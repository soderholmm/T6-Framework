jQuery(function ($) {
  ...
  $(document).on("click touchstart", ".t6-row-options", function (event) {
    event.preventDefault();
    var $rowSettings = $(".t6-row-settings"),
      $dataCols = [];
    if (!$rowSettings.parents().is(".themeConfigModal"))
      $rowSettings.appendTo(".themeConfigModal");
    $("body").addClass("t6-modal-open");
    $rowSettings.find(".t6_padding,.t6_margin").hide();
    $rowSettings.find(".t6_extra_class,.t6_layout_sticky").show();
    $rowSettings.show();
    $(".row-active").removeClass("row-active");
    $parent = $(this).closest(".t6-layout-section");
    /*
    ...
    T6Layout.initDataPaddingandMargin($dataSection);
    $parent.find(".t6-layout-col").each(function () {
      $dataCols.push($(this).data());
    });
    var $clone = $(".t6-row-setting");
    $clone.find(".config-section").html(T6Layout.configSection($parent, "def"));
    var $divlayout = '<ul class="t6-admin-layout">';
    $divlayout +=
      '<li><span class="t6-admin-layout-structur active" title="Layout Default" data-tooltip="Layout Default">Layout Default</span></li>';
    $divlayout +=
      '<li><span class="t6-admin-responsive" title="Layout responsive" data-tooltip="Layout responsive">responsive</span></li>';
    $divlayout += "</ul>";
    $clone.find(".config-section").prepend($divlayout);
    $clone.find(".t6-col-remove").each(function () {
    ...
    $clone.find(".t6-group-general").addClass("active");
    $clone.find(".t6-input-cols").data("layout", $dataSection.layout);
    var cols = $clone.find(".t6-layout-col").length;
    if ($clone.find(".alert.alert-warning").length) {
      $clone.find(".alert.alert-warning").remove();
      $clone.find(".t6-modal-footer").removeClass("disabled");
    }
    $clone.find(".t6-input-color").each(function () {
      if (!$(this).hasClass("hasinit")) {
        $(this).addClass("minicolors hasinit");
      }
    });
    var oldLayout = "";
    $clone.find(".t6-layout-col").each(function (idx) {
      var $idx = $(this).find(".t6-admin-layout-vis").data("idx");
    ...
    if (typeof $container == "undefined" || !isNaN($container)) {
      ...
      $(".t6-layout-container").val("container");
    } else {
      $('[data-container="' + $container + '"]').addClass("active");
      $(".t6-layout-container").val($container);
    }
    ...
    var countCol = $clone.find(".t6-layout-col").length,
      countPos = $(".t6-admin-layout-hiddenpos").find(".hide").length;
    if (countCol != countPos) {
      $(".t6-admin-layout-hiddenpos").addClass("haspos");
    }
    $clone.find(".fuildwidth .btn").each(function () {
      T6Layout.t6Container($(this));
    });
    T6Layout.initOverlay();
    ...
    $(".t6-admin-dv-def").click();
    T6Layout.resizeElement();
  });

  // Open Column settings Modal
  $(document).on("click touchstart", ".t6-column-options", function (event) {
    event.preventDefault();
    var $colSettings = $(".t6-cols-settings");
    if (!$colSettings.parents().is($(".themeConfigModal")))
      $colSettings.appendTo($(".themeConfigModal"));
    $("body").addClass("t6-modal-open");
    $colSettings.show();
    $(".t6-layout-col").removeClass("column-active");
    $parent = $(this).closest(".t6-layout-col");
    $parent.addClass("column-active");

    $(".t6-cols-inner")
      .find("select.t6-layout")
      .each(function () {
        $(this).chosen("destroy");
      });

    var $clone = $(".t6-cols-inner");
    $clone.find(".tab-pane").removeClass("active show").addClass("fade");
    $clone.find("#general").removeClass("fade").addClass("show active");
    $clone.find(".t6-layout").each(function () {
      var $that = $(this),
    ...
    $clone.find("select.t6-layout").each(function () {
      ...
      var $typeAct = $("." + typeName).find("select.t6-layout");
  ...
  $(document).on("click", ".t6-layout-column .btn", function (e) {
    var cols = $(".t6-layout-xresize").find(".t6-layout-col").length;
    if ($(this).hasClass("t6-col-add") && cols < 12) {
      cols = cols + 1;
    }
    if ($(this).hasClass("t6-col-remove") && cols > 1) {
      cols = cols - 1;
    }
    var $newLayout = T6Layout.layoutBuilder(cols);

    $(".config-section").find(".t6-layout-xresize").html($newLayout);

    $(".config-section").find(".t6-column-options").remove();
    $(".config-section").find(".ui-resizable-handle").remove();
    $(".config-section").find(".t6-layout-col").removeAttr("data-sortableitem");
    $(".config-section").find(".t6-layout-col").addClass("t6-layout-unit");
    var $jposHide = "";
    $(".config-section")
      .find(".t6-layout-col")
      .each(function (index) {
        ...
        $(".config-section")
          .find(".t6-admin-layout-hiddenpos")
          .find(".pos-hidden")
          .remove();
        $jposHide +=
    ...
    $(".config-section").find(".t6-admin-layout-hiddenpos").append($jposHide);
    $(".t6-input-cols").val(cols);
    T6Layout.resizeElement();
  });

  // Save Row Column Settings
  $(document).on("click", ".t6-settings-apply", function (event) {
    event.preventDefault();
    ...
    switch (flag) {
          ...
          .each(function () {
            ...
            if ($attrname == "name") {
              ...
              if (nameVal == "" || nameVal == null) {
                $(".row-active .t6-section-title").text("Section");
              } else {
                $(".row-active .t6-section-title").text($this.val());
              }
            }
            if ($attrname == "cols") {
              cols = $this.val();
              ...
              var $options = {
                classes: "t6-layout-col",
                layout: "column",
              };
              T6Layout.layoutArr($options);
            }
            if ($attrname == "padding") {
      ...
      default:
        T6Admin.messages("You are doing somethings wrongs. Try again", "error");
    }
    $(".t6-col").removeClass("t6-layout-unit");
    $(".t6-admin-layout-vis").hide();
    $(".t6-col-remove").addClass("hidden");
    $(".row-active").removeClass("row-active");
    $(".column-active").removeClass("column-active");
    // $('.t6-modal-overlay').remove();
    $(".themeConfigModal").children().not(".t6-message-container").hide();
    $("body").removeClass("t6-modal-open");
    var $dataLayout = T6Layout.getGeneratedLayout();
    $("input.t6-layouts").val(JSON.stringify($dataLayout)).trigger("change");
    // T6Admin.t6Ajax($dataLayout,'SaveLayout');
  });
  // Cancel Modal
  $(document).on(
    "click",
    ".t6-settings-cancel, .action-t6-modal-close",
    function (event) {
      event.preventDefault();
      // $('.t6-modal-overlay').remove();
      $(".themeConfigModal").children().not(".t6-message-container").hide();
  ...
  // add row
  $(document).on("click", ".t6-add-row", function (event) {
    event.preventDefault();

    var $parent = $(this).closest(".t6-layout-section"),
      $rowClone = $("#t6-layout-section").clone(true);

    $rowClone.addClass("t6-layout-section").removeAttr("id");
    $($rowClone).insertAfter($parent);

    T6Layout.jqueryUiLayout();
  });

  // Remove Row
  $(document).on("click", ".t6-remove-row", function (event) {
    event.preventDefault();
    var $that = $(this);
    T6Admin.Confirm(
      T6Admin.langs.t6LayoutRowConfirmDel,
      function (ans) {
        if (ans) {
          $that.closest(".t6-layout-section").slideUp(500, function () {
            var countSection =
              $(".t6-layout-builder").find(".t6-layout-section").length;
            if (countSection == 2) {
              $that
                .closest(".t6-layout-section")
                .find(".t6-add-row")
                .trigger("click");
            }
            $that.closest(".t6-layout-section").remove();
            T6Layout.layoutApply();
            T6Admin.Messages(T6Admin.langs.t6LayoutRowDeleted, "message");
          });
        } else {
          return false;
        }
  ...
  /*Option Group*/
  $(document).on("click", ".nav-item.t6-group", function (event) {
    event.preventDefault();
  ...
  $(document).on("click", ".t6-layout-styles-settings", function (e) {
    $("#imageModal_t6layout_layout_media").appendTo("body");
    $("#imageModal_t6layout_layout_media").css({ "z-index": "3000" });
  });
  ...
  //     console.log(this);
  // });
  $(document).on("click", ".t6-admin-layout-vis", function (e) {
    var $device = T6Layout.getDeviceActive();
    var jPos = $(".t6-input-cols").data("pos_" + $device) | "";
    ...
    var $col = $(this).parents(".t6-layout-col");
    $col.addClass("pos-hidden-" + $device);
    $(".config-section")
      .find('[data-item_vis="' + $datashow + '"]')
      .removeClass("hide");
    $(".t6-input-cols").data("pos_" + $device, jPos);
    if (!$(".t6-admin-layout-hiddenpos").hasClass("haspos")) {
      $(".t6-admin-layout-hiddenpos").addClass("haspos");
    }
  });
  $(document).on(
    "click",
    ".t6-admin-layout-hiddenpos .pos-hidden",
    function () {
      ...
      var countCol = $(".config-section").find(".t6-layout-col").length,
        countPos = $(".t6-admin-layout-hiddenpos").find(".hide").length;
      if (countCol == countPos) {
        $(".t6-admin-layout-hiddenpos").removeClass("haspos");
      }
    }
  );
  $(document).on("click", ".t6-admin-dv-auto", function (e) {
    $device = T6Layout.getDeviceActive();
    if ($device != "def") {
      $(".config-section")
        .find(".t6-layout-col")
        .each(function () {
    ...
    $(".config-section")
      .find(".t6-admin-layout-devices")
      .find('[data-device="' + $device + '"]')
      .click();
  });
  $(document).on("click", ".t6-admin-dv-reset", function (e) {
    $device = T6Layout.getDeviceActive();
    if ($device != "def") {
      $(".config-section")
        .find(".t6-layout-col")
      ...
      var old_layout = $(".t6-input-cols").data("old_layout");
  ...
  $(document).on(
    "click",
    ".t6-admin-dv-clear, .t6-admin-dv-none",
    function (e) {
      $device = T6Layout.getDeviceActive();
      if ($device != "def") {
        $(".config-section")
  ...
  $("body").on("click", ".t6-layout-styles-settings", function () {
    $(".t6-modal.t6-row-setting").addClass("hidden");
  });
  $("body").on("hidden.bs.modal", ".modal", function () {
    if ($(".t6-modal.t6-row-setting").hasClass("hidden")) {
      $(".t6-modal.t6-row-setting").removeClass("hidden");
    }
  });
  $("body").on("click", ".t6-col-remove", function (e) {
    var parentsCol = $(this).parents(".t6-layout-xresize");
    var lastCol = parentsCol.find(".t6-layout-col").length;
    if (lastCol == 1) {
      T6Admin.Messages("cant remove last column!!!!", "error");
    } else {
      var elemChange = "";
      ...
      var hasPoss = parentsCol.next(".t6-admin-layout-hiddenpos");
      var checkColIdx = parentsCol.find(".t6-layout-col").toArray();
      var colIdx = checkColIdx.indexOf($(this).parents(".t6-layout-col")[0]);
      ...
      if (checkColIdx.length == 2) {
        elemChange.find(".t6-col-remove").addClass("hidden");
      }
      removeColClass(elemChange);
      elemChange.addClass("col-md");
      elemChange.data("col", "auto");
      elemChange.find(".t6-column-title").text("auto");
      var posIdx = $(this).parents(".t6-layout-col").data("idx");
      hasPoss.find('[data-item_vis="' + posIdx + '"]').remove();
      $(this).parents(".t6-layout-col").remove();
    }
  });
  $(document).on("click", ".t6-admin-layout-structur", function (e) {
    $(this).addClass("active");
    $(".t6-admin-responsive").removeClass("active");
    $(".t6-row-settings").find(".t6_padding,.t6_margin").hide();
    $(".t6-row-settings").find(".t6_extra_class,.t6_layout_sticky").show();
    $(".t6-layout-column").show();
    $(".t6-layout-xresize .t6-col-remove").removeClass("hidden");
    $(
      ".t6-admin-layout-devices , .t6-admin-dv-clear,.t6-admin-layout-vis"
    ).hide();
    $(".t6-admin-dv-def").click();
  });
  $(document).on("click", ".t6-admin-responsive", function (e) {
    $(this).addClass("active");
    $(".t6-admin-layout-structur").removeClass("active");
    $(".t6-layout-column").hide();
    $(".t6-col-remove").addClass("hidden");
    $(".t6-admin-layout-devices , .t6-admin-dv-clear").show();
    $(".config-section").find(".t6-admin-layout-vis").show();
    $(".t6-row-settings").find(".t6_padding,.t6_margin").show();
    $(".t6-row-settings").find(".t6_extra_class,.t6_layout_sticky").hide();
    $(".t6-admin-dv-xl").click();
    var device = T6Layout.getDeviceActive();
    T6Layout.initPaddingMarginValue(device);
  });
  // update padding of Device responsive
  $(document).on("keyup", "#t6layout_padding", function (e) {
    var device = T6Layout.getDeviceActive();
    if (device == "def") device = "xl";
    var paddingData = $(this).val();
    $("#t6layout_padding").data("padding_" + device, paddingData);
  });
  // update margin of Device responsive
  $(document).on("keyup", "#t6layout_margin", function (e) {
    var device = T6Layout.getDeviceActive();
    if (device == "def") device = "xl";
    var marginData = $(this).val();
    $("#t6layout_margin").data("margin_" + device, marginData);
  });
  //init opacity
  $(document).on("change", "input#opacityVal", function (e) {
    ...
    if (0 <= parseInt($valueOp) && parseInt($valueOp) <= 1) {
      $("#t6layout_opacity").val($(this).val());
      $(".slider-bg-lower").css({ width: $flex + "%" });
    }
  });
  $(document).on("change mousemove", "#t6layout_opacity", function (e) {
  ...
  $(document).on("change", ".overlay_type", function (e) {
    var $overlay_type = $(this).find(".t6-layout").getInputValue();
    console.log('====================================');
    ...
    T6Layout.initOverlayReadonly($overlay_type);
  });
  $(document).on(
    "change",
    ".video_type input, .image_type .field-media-input, .file_type input",
    function (e) {
      var $overlay_type = $(".overlay_type").find(".t6-layout").getInputValue();
      console.log('====================================');
      console.log($overlay_type);
      console.log('====================================');
      T6Layout.initOverlayReadonly($overlay_type);
  ...
  $("#t6layoutcol_edit_block").hide();
  $(`<div id="t6layoutcol_block_editor" style="height: 500px;"></div>`).insertAfter($("#t6layoutcol_edit_block"));

  //edit block
  $('.t6-btn[data-action="block.edit"]').click(async function () {
    $(".t6-edit-block-remove").show();
    $(".t6-block-name").hide();
    // load current block
    var blockname = $('[name="t6layoutcol[block]"]').val();

    if (!blockname) {
      T6Admin.Messages("You must select a block to edit!", "error");
      return false;
    }

    $(".t6-block-name").find("input").val(blockname);

    const editor = await T6CodeEditor.get({
      name: 'blockEditor',
      container: '#t6layoutcol_block_editor',
      language: 'html',
    });
    ...
    try {
      const url = location.pathname + "?option=com_ajax&plugin=t6&format=html&t6do=getblock&id=" + tempId;
      ...
      if (pos == "ovr") {
        $(".t6-edit-block-remove").html(
          '<span class="fal fa-undo"></span> Restore Default'
        );
        $(".t6-edit-block-remove").data("local", "ovr");
      } else if (pos == "loc") {
        $(".t6-edit-block-remove").html(
          '<span class="fal fa-trash-alt"></span>Delete Block'
        );
        $(".t6-edit-block-remove").data("local", "loc");
      } else {
        $(".t6-edit-block-remove").hide();
      }
    } catch (error) {
  ...
  // action add block
  $('.t6-btn[data-action="block.add"]').click(async function () {
    if ($(".control-group.t6-block-name").find(".alert.alert-warning").length) {
      $(".control-group.t6-block-name").find(".alert.alert-warning").remove();
      $(".t6-edit-block-footer").removeClass("disabled");
    }
    $(".t6-edit-block-remove").hide();
    $(".t6-block-name").find("input").val("");
    $(".t6-block-name").show();
  ...
  var hideColModalSettings = function ($action) {
    ...
    var tab_editblock = $(".t6-modal-col").find("#editblock");
    $(".t6-cols-setting")
      .find(".t6-modal-footer, .t6-modal-header-title, .action-t6-modal-close")
      .hide();
    $(".t6-modal-col")
      .find("#general")
      .removeClass("show active")
    ...
    $(".t6-cols-setting")
      .find(
        ".t6-edit-block-footer, .t6-edit-block-title, .t6-modal-block-close"
      )
      .show();
  };
  ...
  //cancel edit block
  $("body").on("click", ".t6-edit-block-cancel", function (e) {
  ...
  $("body").on("click", ".t6-edit-block-save", function (e) {
    e.preventDefault();

    var blockname = $("#t6layoutcol_new_block").val(),
      blName = blockname.replace(/\s+/g, "_").toLowerCase();
    if (!blockname) {
      T6Admin.Messages(T6Admin.langs.T6BlockNameNone, "error");
      return false;
    }

    const { blockEditor } = T6CodeEditor.instances;
    const block = blockEditor.getValue();

    T6Layout.saveBlock(blName, block);
  });
  // remove block
  $("body").on("click", ".t6-edit-block-remove", function (e) {
    var localFile = $(".t6-edit-block-remove").data("local");
    e.preventDefault();
    T6Admin.Confirm(
      "Are you sure?",
      function (ans) {
        if (ans) {
          var deleteFile =
            location.pathname +
            "?option=com_ajax&plugin=t6&format=html&t6do=removeblock&id=" +
            tempId;
          var blockname = $("#t6layoutcol_new_block").val();
          $.post(deleteFile, { block: blockname }).then(function (data) {
            if (localFile == "loc") {
              var $selectBox = document.getElementById("t6layoutcol_block"),
                $selected = $selectBox.selectedIndex;
              $selectBox.remove($selected);
              $("#t6layoutcol_block").val("").trigger("liszt:updated");
              $(".t6-modal-col")
                .find("#editblock")
                .removeClass("show active")
                .addClass("fade");
              ...
              $(".t6-cols-setting")
                .find(
                  ".t6-modal-footer, .t6-modal-header-title, .action-t6-modal-close"
                )
                .show();
              $(".t6-cols-setting")
                .find(".t6-edit-block-footer,.t6-edit-block-title")
                .hide();
            } else {
              var $data = data["data"];
              const { blockEditor } = T6CodeEditor.instances;
  ...
  $(document).on("click", ".t6-modal-block-close", function (e) {
    $(".t6-edit-block-cancel").trigger("click");
  });
  //check exist name block for create new block
  $(document).on("change", "#t6layoutcol_new_block", function (e) {
    var newName = $(this).val(),
      $checkName = $("#t6layoutcol_block").find(
        'option[value="' + newName + '"]'
      ).length;
    if ($checkName) {
      $(".control-group.t6-block-name")
        .find(".control-group-inner")
        .append(
          '<div class="alert alert-warning">The name block has exist!</div>'
        );
      $(".t6-edit-block-footer").addClass("disabled");
    } else {
      if (
        $(".control-group.t6-block-name").find(".alert.alert-warning").length
      ) {
        $(".control-group.t6-block-name").find(".alert.alert-warning").remove();
        $(".t6-edit-block-footer").removeClass("disabled");
      }
  ...
  $(document).on("change", "#t6layout_name", function (e) {
    var $val = $(this).val(),
      $name_section = [];
    var $btn = $(this);
    $(".t6-layout-builder")
      .find(".t6-layout-section")
      .each(function () {
  ...
  $(document).on("click", ".t6-layout-palettes .pattern", function (e) {
    e.preventDefault();
    e.stopPropagation();
    T6Layout.updatePaletteColor();
    ...
    $(".t6-input-color_pattern").val($data.class);
    //init preview
    ...
    prewviewEl.find(".t6-palette-color-spec").each(function () {
  ...
  $(document).on("click", ".t6-admin-layout-devices .btn", function (e) {
    var $elem = $(this);
    $(".t6-admin-layout-devices .active").removeClass("active");
    $elem.addClass("active");
    var $device = $elem.data("device");
    $(".t6-layout-reset .btn").hide();
    T6Layout.initPaddingMarginValue($device);
    if ($device != "def") {
      $(".config-section").find(".t6-admin-layout-vis").show();
      $(".config-section")
        .find(".t6-layout-col")
        .each(function (idx) {
          T6Layout.posHide($(this), idx, $device);
        });
    } else {
      $(".config-section").find(".t6-admin-layout-vis").hide();
      $(".config-section")
        .find(".t6-admin-layout-hiddenpos .pos-hidden")
        .addClass("hide");
    }
    if ($device == "xs") {
      $(".t6-admin-dv-auto").show();
      $(".t6-admin-dv-none").show();
    } else if ($device == "xl" || $device == "lg" || $device == "md") {
      $(".t6-admin-dv-reset").show();
      $(".t6-admin-dv-auto").show();
    } else {
      $(".t6-admin-dv-auto").show();
    }
    ...
    $elem
      ...
      .each(function (idx) {
        ...
        $t6Col.removeClass(function (index, className) {
          return (className.match(/(^|\s)col-\S+/g) || []).join(" ");
        });
        $t6Col.removeClass("col");
        if ($device == "def") {
          if ($device == "def") $deviceDefault = "md";
          var dataCol = $t6Col.data("col");
          if (dataCol == "auto") {
            $t6Col.addClass("col-" + $deviceDefault);
          } else {
            $t6Col.addClass("col-" + $deviceDefault + "-" + dataCol);
          }
          $t6Col.find(".t6-column-title").text(dataCol);
        } else {
          $t6Col.addClass(classCol);
          if ($colLayout && $colLayout != "none") {
            $t6Col.find(".t6-column-title").text($colLayout);
          } else if ($colLayout == "") {
            $t6Col.find(".t6-column-title").text("auto");
          } else {
            $t6Col.find(".t6-column-title").text("none");
          }
        }
      });
    var countCol = $(".config-section").find(".t6-layout-col").length,
  ...
  // block custom css
  $(document).on("click", ".t6-settings-block-css", function (e) {
    var theme = $("#attrib-themeConfig.dark").length ? "monokai" : "default";
    var $cssModal = $(document).find(".t6-block-css-modal");
    if ($cssModal.length && !$cssModal.parents().is(".themeConfigModal"))
      $cssModal.appendTo(".themeConfigModal");
    var blockName = $("#t6layout_name").val().replace(/\s/g, "-").toLowerCase();
    // load current custom css
    var url =
      location.pathname +
      "?option=com_ajax&plugin=t6&format=json&t6do=blockcss&id=" +
      tempId;
    $.post(url, { task: "getcss", name: blockName }).then(function (css) {
      ...
      var $rowSettings = $(".t6-row-settings");
      $rowSettings.hide();
      $("#t6_block_css").text(css);
      var textArea = $("#t6_block_css").get(0);
      if (!block_css_editor) {
  ...
  $(document).on("click", ".block-css-editor-apply", function (e) {
    ...
    var blockName = $("#t6layout_name").val().replace(/\s/g, "-").toLowerCase();
    T6Admin.Messages("Saving & Compiling ...", "status");
    // load current custom css
    var url =
      location.pathname +
      "?option=com_ajax&plugin=t6&format=json&t6do=blockcss&id=" +
    ...
    $.post(url, { task: "save", name: blockName, blockcss: scss }).then(
      function (data) {
        if (data.error) {
          T6Admin.Messages(data.error, "error");
        } else {
          $(".t6-block-css-modal").hide();
          $(".t6-row-settings").show();
          T6Admin.Messages("Save & compile successfully!");
        }
      }
    );
  });
  $(document).on("click", ".t6-block-css-cancel", function (e) {
    e.preventDefault();
    $(".t6-block-css-modal").hide();
    $(".t6-row-settings").show();
  });
});
var T6Layout = window.t6Layout || {};
!(function ($) {
  $.extend(T6Layout, {
    ...
    initPaddingMarginValue: function (device) {
      ...
      var paddingTitle = $(".t6_padding").find("#t6layout_padding-lbl").text();
      var tooltip =
      ...
      paddingTitle = paddingTitle
        ...
        .trim();
      $(".t6_padding")
        .find("#t6layout_padding-lbl")
        .html(paddingTitle + "(" + device + ") " + tooltip);

      // update title margin
      var marginTitle = $(".t6_margin").find("#t6layout_margin-lbl").text();
      var tooltip =
        '<span class="hasTooltip fal fa-question-circle" data-original-title="" title=""></span>';
      marginTitle.trim();
      marginTitle = marginTitle
        ...
        .trim();
      $(".t6_margin")
        .find("#t6layout_margin-lbl")
        .html(marginTitle + "(" + device + ") " + tooltip);
      // update value margin
      var paddingData = $("#t6layout_padding").data("padding_" + device) || "";
      $("#t6layout_padding").val(paddingData);
      // update value margin
      var marginData = $("#t6layout_margin").data("margin_" + device) || "";
      $("#t6layout_margin").val(marginData);
    },
    initDataPaddingandMargin(dataSection) {
      $("#t6layout_padding").data("padding_xl", dataSection["padding_xl"] ? dataSection["padding_xl"] : "");
      $("#t6layout_padding").data("padding_lg", dataSection["padding_lg"] ? dataSection["padding_lg"] : "");
      $("#t6layout_padding").data("padding_md", dataSection["padding_md"] ? dataSection["padding_md"] : "");
      $("#t6layout_padding").data("padding_sm", dataSection["padding_sm"] ? dataSection["padding_sm"] : "");
      $("#t6layout_padding").data("padding_xs", dataSection["padding_xs"] ? dataSection["padding_xs"] : "");
      $("#t6layout_margin").data("margin_xl", dataSection["margin_xl"] ? dataSection["margin_xl"] : "");
      $("#t6layout_margin").data("margin_lg", dataSection["margin_lg"] ? dataSection["margin_lg"] : "");
      $("#t6layout_margin").data("margin_md", dataSection["margin_md"] ? dataSection["margin_md"] : "");
      $("#t6layout_margin").data("margin_sm", dataSection["margin_sm"] ? dataSection["margin_sm"] : "");
      $("#t6layout_margin").data("margin_xs", dataSection["margin_xs"] ? dataSection["margin_xs"] : "");
    },
    // Column Layout Arrange
    layoutArr: function (options) {
      var col = [],
        colNew = [],
        $gparent = $(".t6-layout-section.row-active"),
        colAttr = [],
        newLayout = [];
      ...
      for (var i = 0; i < newLayout.length; i++) {
        ...
        new_item +=
          '<span class="t6-column-title">' + colAttr[i].name + "</span>";
        new_item +=
          '<span class="t6-col-remove" title="Remove column" data-content="Remove column"><i class="fal fa-minus"></i> </span>';
        new_item +=
          '<span class="t6-admin-layout-vis" data-idx="' +
          i +
    ...
    saveBlock: function (block, data, task) {
      ...
      $.post(url, { name: block, data: data }).then(function () {
        ...
        $(".t6-cols-setting")
          ...
          .hide();
        T6Admin.Messages(T6Admin.langs.T6LayoutSaveBlock, "message");
      });
    },
    // Column Layout Arrange
    layoutBuilder: function (cols) {
      ...
      colAttr = [];
      $gparent.find(".t6-layout-col").each(function (i, val) {
        col[i] = $(this).html();
        var colData = $(this).data();
      ...
      for (var i = 0; i < cols; i++) {
        ...
        new_item +=
          '<div class="t6-col t6-layout-col col-md" ' + dataAttr + ">";
        if (col[i]) {
          new_item += col[i];
        } else {
          new_item += '<div class="col-inner clearfix">';
          ...
          new_item +=
            ...
            '" title="Click here to hide this position on current device layout" style="display:none;"><i class="fal fa-eye"></i></span>';
          new_item +=
            '<a class="t6-column-options" href="#"><i class="fal fa-cogs"></i></a>';
          new_item += "</div>";
          var pos_hide = $(
            "<span class='pos-hidden hide' data-item_vis='" +
            i +
          ...
          $(".t6-admin-layout-hiddenpos").append(pos_hide);
        }
        new_item += "</div>";
      }
      return new_item;
    ...
    layoutApply: function () {
      ...
      typelist.saved = false;
      var $dataLayout = T6Layout.getGeneratedLayout();
      $(".t6-layouts").val(JSON.stringify($dataLayout)).trigger("change");
      // T6Admin.t6Ajax($dataLayout,'SaveLayout');
    },
    jqueryUiLayout: function () {
      ...
      $(".t6-layout-section").find(".row").rowSortable("layout");
    },
    ...
    getGeneratedLayout: function () {
      ...
      $("#t6-layout-builder")
        ...
        .each(function (index) {
          ...
          $row.find(".t6-layout-col").each(function (index) {
            var $column = $(this),
              colIndex = index,
              colObj = $column.data();
    ...
    getDeviceActive: function () {
      var $device = $(".t6-admin-layout-devices .active").data("device");
      if (["xl", "lg", "md", "sm", "xs"].indexOf($device) != "-1") {
        return $device;
      }
      return "def";
    },
    posHide: function ($t6Col, index, $device) {
      var $colData = $t6Col.data(),
        hideDevice = "hidden_" + $device;
      var pos_col = $colData[hideDevice];
      if (typeof pos_col == "undefined") {
        $t6Col.data("hidden_" + $device, "");
        pos_col = "";
      }
      if (pos_col) {
        $t6Col.addClass("pos-hidden-" + $device);
        $(".config-section")
    ...
    resizeElement: function () {
      ...
      var resizableEl = $(".t6-layout-col.t6-layout-unit"),
        device = T6Layout.getDeviceActive(),
        columns = 12,
        SecLayout = "",
        ...
        updateClass = function (el, col, dv) {
          ...
          var $idx = el.data("idx");
          var layoutCol = $(".config-section").find(".t6-layout-col");
    ...
    initPreview: function () {
      // init toggle value
      var $previewtoggle = $("#t6-preview-layout"),
      ...
      $(".t6-color-pattern")
        ...
        .each(function () {
          $(this).on("click", function (e) {
            e.preventDefault();
            e.stopPropagation();
            $(".t6-color-pattern").find(".active").removeClass("active");
            $(this).addClass("active");
            var $data = $(this).data();
            $(".t6-input-color_pattern").val($data.class);
          });
    ...
    initOverlay: function () {
      ...
      $("body")
        ...
        .hide();
      $(".opacity").find(".t6-layout").val($opacity).trigger("mousemove");
      if ($over_type != "") {
        $("body")
    ...
    configSection: function ($elem, $device) {
      ...
      $clone.find(".row").addClass("t6-layout-xresize");
      $clone.find(".t6-layout-col").addClass("t6-layout-unit");
      var $colHide = "",
        dataCol = [];
      $section.find(".t6-layout-col").each(function () {
        dataCol.push($(this).data());
      });
      $clone.find(".t6-layout-col").each(function (index) {
        if (typeof dataCol[index].col == "undefined")
          dataCol[index].col = "auto";
        $(this).find(".t6-column-title").text(dataCol[index].col);
        $colHide +=
          '<span class="pos-hidden hide" data-item_vis="' +
          index +
          '" data-hidden_sm="' +
      ...
      if ($clone.find(".t6-layout-col").length) {
        var $gridClass = "t6-layout-devices";
        var $configGrid =
          '<div class="t6-admin-layout-devices btn-group" style="display:none">';
        $configGrid +=
          '<button style="display:none;" class="btn t6-admin-dv-def active" data-device="def" title="Layout def" data-tooltip="Layout def"><i class="fal fa-desktop"></i></button>';
        $configGrid +=
          '<button class="btn t6-admin-dv-xl active" data-device="xl" title="Layout xl" data-tooltip="Layout xl"><i class="fal fa-desktop"></i></button>';
        $configGrid +=
          '<button class="btn t6-admin-dv-lg" data-device="lg" title="Layout Desktop" data-tooltip="Layout Desktop"><i class="fal fa-desktop"></i></button>';
        $configGrid +=
          '<button class="btn t6-admin-dv-md" data-device="md" title="Layout laptop" data-tooltip="layout laptop"><i class="fal fa-laptop"></i></button>';
        $configGrid +=
          '<button class="btn t6-admin-dv-sm" data-device="sm" title="Tablets" data-tooltip="Tablets"><i class="fal fa-tablet-alt"></i></button>';
        $configGrid +=
          '<button class="btn t6-admin-dv-xs" data-device="xs" title="Phone" data-tooltip="Phone"><i class="fal fa-mobile-alt"></i></button>';
        $configGrid += "</div>";
        var resetAll = '<div class="t6-layout-reset">';
        resetAll +=
          '<span class="btn t6-admin-dv-auto" title="Auto layout" data-tooltip="Auto layout">Set auto</span>';
        resetAll +=
          '<span class="btn t6-admin-dv-clear" style="display:none;" title="Clear layout" data-tooltip="Clear layout">Clear</span>';
        resetAll +=
          '<span class="btn t6-admin-dv-reset" style="display:none;" title="Reset layout" data-tooltip="Reset layout">Reset</span>';
        resetAll +=
          '<span class="btn t6-admin-dv-none" style="display:none;" title="Set none layout" data-tooltip="Set none layout">Set none</span>';
        resetAll += "</div>";
        var addColumn = '<div class="t6-layout-column">';
        addColumn +=
          '<span class="btn t6-col-add" title="Add column" data-content="Add column"><i class="fal fa-plus"></i>Add Column</span>';
        addColumn += "</div>";

        var admAction = $("<div />")
          .addClass("t6-admin-layout-action")
          .append(resetAll, $configGrid, addColumn);
        $clone.prepend(admAction);
        var jcolHide =
          '<div class="t6-admin-layout-hiddenpos" title="Currently hidden positions">' +
          $colHide +
          "</div>";
    ...
    updatePaletteColor() {
      var colorPattern = {},
        $pattern = ".pattern";
      var plLayoutNum = $(".t6-layout-palettes").find(".pattern").not(".none");
      var plThemeNameArr = [];
      var plLayoutNameArr = [];
      ...
      $(".group_palette")
        ...
        .each(function (index) {
          ...
          var layoutPl = $(".t6-layout-palettes").find(
          ...
          if (!layoutPl.length) {
            var plNew = $(`
                        ...
                        `);
            plNew.appendTo($(".t6-layout-palettes"));
          } else {
            layoutPl
              .find(".pattern-inner")
    ...
    if (jversion != 3) {
      ...
      if (_JoomlaGetMedia) {
        Joomla.getMedia = (data, editor, fieldClass) => new Promise((resolve, reject) => {
          ...
          }).then(response => response.json()).then(async response => resolve(await T6ExecTransform(response, editor, fieldClass))).catch(error => reject(error));
        });
        var T6isElement = o => typeof HTMLElement === 'object' ? o instanceof HTMLElement : o && typeof o === 'object' && o.nodeType === 1 && typeof o.nodeName === 'string';
        var T6insertAsImage = async function (media, editor, fieldClass) {
          if (media.url) {
            var {
              rootFull
        ...
        var T6ExecTransform = async (resp, editor, fieldClass) => {
          if (resp.success === true) {
            ...
            if (Joomla.selectedMediaFile.extension && images.includes(media.extension.toLowerCase())) {
              return T6insertAsImage(media, editor, fieldClass);
            }
