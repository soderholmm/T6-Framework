jQuery(function($){
    ...
    $('#attrib-themeConfig').addClass('t6-pane');
    //append themeconfigModal to body
    $('body').append('<form class="themeConfigModal" onsubmit="event.preventDefault()"></form>');
    var $T6message = $('<div />').addClass('t6-message-container');
    $('.themeConfigModal').prepend($T6message);
    $('body.admin').addClass('j'+jversion);
    var paneElement = '.t6-pane';
    T6Admin.initrenderForm();
    ...
    $(paneElement).each(function(){
        ...
        var $pane = $(this),
            ...
            $t6PreviewBar = $('<div class="t6-sidebar-preview" />'),
            $curStyle = '<div class="t6-current-style"><h1 class="temp_title">'+tempStyle+'</h1>'+allTempl+'</div>',
            $toolbarDevice = '<span class="reload-preview"><i class="fal fa-redo-alt"></i>Reload Preview</span><ul class="t6-toolbar toolbar-devices t6-responsive"><li class="active"><span class="default">All Devices</span></li><li ><span class="btn desktop" data-tooltip="Desktop"><i class="fal fa-desktop"></i></span></li><li><span class="btn tablet" data-tooltip="Tablet"><i class="fal fa-tablet"></i></span></li><li><span class="btn mobile" data-tooltip="Mobile"><i class="fal fa-mobile"></i></span></li></ul>',
            $toolbarAction = '<div class="t6-toolbar toolbar-save"><div class="t6-admin-save"><span class="t6-btn btn-md btn-success btn-save" type="button" data-tooltip="Save"><i class="fal fa-save"></i><span class="btn-text">Save</span></span><span class="t6-btn btn-md btn-success dropdown-toggle" type="button" '+attrDrop+'><span class="caret"></span>&nbsp;</span><ul class="dropdown-menu"><li id="t6-admin-btn-save-close"><a href="#">Save &amp; Close</a></li><li id="t6-admin-btn-save-clone"><a href="#">Save as Copy</a></li></ul></div><a href="'+t6_site_root_url+'" title="View site" class="t6-btn btn-md" target="_blank"><i class="fal fa-external-link"></i><span class="btn-text">View site</span></a><span class="t6-btn btn-md btn-icon btn-close" type="button" data-tooltip="Close" title="Close"><i class="fal fa-times"></i></span></div>',

            $t6Sidebar = $('<div />').addClass('t6-sidebar'),
            $t6Switch = $('<div class="t6-switch-theme"/>').append('<ul><li><label for="switch-style"><input id="switch-style" class="t6-input" type="checkbox" value="0" name="t6-switch" /><span>Dark Mode</span></label></li><li><a href="#"><img src="'+t6_site_root_url+'plugins/system/t6/admin/assets/images/t6-logo.png" /></a></li></ul>'),
            $ul = $('<ul />').addClass('t6-sidebar-nav').attr('data-level','1'),
            $t6views = $('<div />').addClass('t6-pn-views').appendTo($pane),
            $topLegends = $pane.find('.top-legend-group');
            $t6PreviewBar.append( $curStyle,$toolbarDevice,$toolbarAction);

            var $i = 1;
        // $t6header.appendTo($t6views);
        $ul.appendTo($t6views);
        $t6Switch.appendTo($t6views);
        $t6views.appendTo($t6Sidebar);
        $pane.append($t6PreviewBar);
        $pane.append($t6Sidebar);

        $topLegends.each(function(){
            var $this = $(this),
                $li = $('<li />').addClass('t6-sidebar-action nav-pane').attr('data-target','pane'+$i);
                $li.appendTo($ul);
                $li.append($this);
            var $legend = $this.find('div.legend'),
                ...
                $t6PageHeader = $('<div class="t6-block-header" />').append('<h4>'+$legend_title.text()+'</h4>'),
                $topGroup = $('<div />');

            $topGroup.addClass('top-group t6-pn-views-container pane').appendTo($t6Sidebar);
            $topGroup.attr({'data-level':'2','data-name':'pane'+$i});
            var attrClass = $legend.attr('class'),t6Class = (attrClass.match(/(^|\s)t6-\S+/g) || []).join(' ');
            if(t6Class){
                $li.addClass(t6Class);
                $topGroup.addClass(t6Class);
            }
            $topGroup.append($t6PageHeader);
            ...
            if(['t6-megamenu','t6-layout-builder','t6-advanced'].indexOf(t6Class.trim()) == -1){
                $params.each(function (){
                    var $ctrlLabel = $(this).find('.control-label'),
                        $param = $(this),
                        $label = $(this).find('label'),
                        $reset = $('<span />').addClass('t6-param-reset').append('<i class="fal fa-undo"></i>');
                    if($label.length){
                    ...
                    $reset.hide();
                    /*if(!$(this).find('.t6-checkbox').length){
                        $ctrlLabel.append($reset);
                    }*/
                    if(t6Class.trim() == 't6-general'){
                        $ctrlLabel.append($reset);
                    }
            ...
            $subLegends.each(function() {
                ...
                var $checkBoxs = $params.find('div.t6-checkbox');
            ...
            var $toggle = $this.find('.t6-group-toggle');
            if ($toggle.length) {
                $toggle.appendTo($t6PageHeader); //.find('input').trigger('change');
            }
        ...
        var $t6Title = $('#style-form').find('div:first'),
            $titleTemplate = $t6Title.find('.control-group').addClass('t6-tpl-title'),
            $detailGroup = $('.t6-detail').find('.sub-group-inner:first');

            $t6Title.remove();
            var $detailsEl;
            if(T6Admin.jversion >= '4'){
                $detailsEl = $('#details .col-lg-3').find('.control-group').addClass('t6-tpl-default-style');
            }else{
                $detailsEl = $('#details .span3').find('.control-group').addClass('t6-tpl-default-style');
            }
            $detailGroup.prepend($detailsEl);
            $detailGroup.prepend($titleTemplate);
            $('.tpl-preview').parents('.control-group').addClass('t6-tpl-detail');
            $('#jform_template-lbl').parents('.control-group').hide();
            //$('#details').appendTo($('#custom-style-preview'));
        $('#assignment').appendTo($('.t6-pn-views-container.t6-assignment'));
        $t6Sidebar.find('#assignment').addClass('t6-assignment').removeClass('tab-pane');
    });

    $('body').on('click', '.t6-sidebar .nav-pane', function(e) {
        var $a = $(this),
            $parentPane = $a.closest('.t6-sidebar-nav'),
            $sidepane = $parentPane.closest('.t6-sidebar'),
        ...
        $sidepane.width($targetPane.outerWidth()+$('.t6-pn-views').outerWidth());
        var t6sidebarWidth = $targetPane.outerWidth() + $('.t6-pn-views').outerWidth();
        if($a.hasClass('t6-assignment')){
            $('#custom-style-preview').addClass('hide');
            $sidepane.width('100%');
        }else{
            $('#custom-style-preview').removeClass('hide');
            $sidepane.width(t6sidebarWidth);
    ...
    $('body').on('click', '.t6-sidebar .back', function() {
        var $a = $(this),
            $sidepane = $a.closest('.t6-sidebar'),
    ...
    var custom_styles = {},
        $all_inputs = $('.t6-sidebar').find('input, textarea, select'),
        $custom_colors = $all_inputs.filter('.minicolors'),$all_global_inputs = $('.t6-sidebar').find('input[name^="jform[param"], textarea[name^="jform[param"], select[name^="jform[param"], #jform_title, #jform_home');
    custom_styles.baseUrl = window.t6_site_root_url;
    ...
    var addClassBtnSave = function($action){
        var $btn = $('.t6-admin-save .btn-save');
        if ($action) {
             $btn.addClass('t6-save-active');
        } else {
            $btn.removeClass('t6-save-active');
        }
    };
    
    setTimeout (function() {
        // switch format for minicolor to rgb when opacity less than 1
        // T6Admin.initMinicolors($custom_colors);
    }, 500);
    ...
    $('.themeConfig').find('.t6-sidebar').css({'z-index':'11'});
    ...
    $('body').on('click','.toolbar-save .btn-close',function(){
        ...
        if(tplhelper.changed == 1){
            T6Admin.Confirm(T6Admin.langs.butonCloseConfirm, function(ans){
                if (ans) {
    ...
    $('body').on('click','.t6-admin-save .btn-save',function(e){
        e.preventDefault();
        $('#tplhelper').val(JSON.stringify({}));
        ...
        if($('#system-message-container').length){
            ...
            $('.t6-message-container').html($('#system-message-container').html());
        }
    });
    $(window).ready(function(){
        if($('#system-message-container').find('.alert').length){
            ...
            window.setTimeout(function() {
                $(".t6-message-container .alert").fadeTo(500, 0).slideUp(500, function(){
                    $(this).remove();
                });
            }, 4000);
        }
				// add class to sublayout
				$(".t6-sub-layout").closest('.control-group').addClass('t6-sub-group-layout');
    });
    $('body').on('click','#t6-admin-btn-save-close',function(e){
        e.preventDefault();
        $('#tplhelper').val(JSON.stringify({}));
    ...
    $('body').on('click','#t6-admin-btn-save-clone',function(e){
        e.preventDefault();
        $('#tplhelper').val(JSON.stringify({}));
    ...
    //responsive iframe
    $('body').on('click', '.t6-responsive li span', function(){
        var $btn = $(this),
            $style = '',
            $sizeScreen = $btn.attr('class').split(' ')[1];
            $('.t6-responsive li').removeClass('active');
    ...
    $(document).on('change',"#typelist_site_dont_use_google_font",function(e){
        ...
        if(fontOnOff == false) {
            T6Admin.disGgFont('enb');
            return;   
        }
        T6Admin.Confirm(T6Admin.langs.T6loadGoogleFontConfirm,function(ans){
            if(ans){
                $('.google-font-input').val('inherit');
                T6Admin.disGgFont('dis');
    ...
    $(document).on('change',"#typelist_site_megamenu_typo_onoff",function(e){
        ...
        if(navigationOnOff){
            $('.group_styles_font').find('.sub-group-params').find('.control-group.megamenu-setting').not('.t6-checkbox').slideDown();
        }else {
            $('.group_styles_font').find('.sub-group-params').find('.control-group.megamenu-setting').not('.t6-checkbox').slideUp();
        }
    });
    $(document).on('change',"#typelist_site_heading_typo_onoff",function(e){
        ...
        if(headingOnOff){
            $('.group_styles_font').find('.sub-group-params').find('.control-group.heading-setting').not('.t6-checkbox').slideDown();
        }else {
            $('.group_styles_font').find('.sub-group-params').find('.control-group.heading-setting').not('.t6-checkbox').slideUp();
        }
    });
    ...
    $(document).on('click','.action-t6-modal-confirm-close', function(e) {
        $(".t6-confirm-settings").hide();
    });
...
!function ($) {
    $.extend(T6Admin, {
        ...
        t6Ajax: function($data,$task){
            $.ajax({
                type: 'POST',
                url: t6_ajax_url+'index.php?option=com_ajax&plugin=t6&format=json&t6do='+$task+'&id='+tempId,
        ...
        initrenderForm: function(){
            $('.control-group').each(function(){
                ...
                var field = "";
                if(!attrClass){
                    attrClass = $(this).find('.control-label').find('.t6-spacer').attr('class');
                    field = "spacer";
                }
        ...
        Messages: function(mesg,type){
            ...
            var $mesgEl = '<div class="alert '+$cls_mesg+'"><button type="button" '+attrDismiss+' class="t6-btn btn-icon close"><i class="fal fa-times"></i></button><div>'+mesg+'</div></div>';
            if(type != 'error'){
                $('.t6-message-container').prepend($mesgEl);
                time = window.setTimeout(function() {
                    $(".t6-message-container .alert").not('.alert-error').fadeTo(500, 0).slideUp(500, function(){
                        $(this).remove();
                    });
                }, 4000);
            }else if(type == 'error'){
                if(typeof time != 'undefined') window.clearTimeout(time); 
                $(".t6-message-container .alert").remove();
                $('.t6-message-container').prepend($mesgEl);
                if(jversion != 3){
                    //on joomla4 need to add event on close button
                    $('.t6-message-container').off('click').on('click','.close',function(){
        ...
        Confirm: function(message, handler,$type){
            ...
            if($type == 'Save'){
                 $("#t6-confirm").find('.btn-no').text('No');
                 $("#t6-confirm").find('.btn-yes').text('Save');
            }else{
                $("#t6-confirm").find('.btn-no').text('No');
                 $("#t6-confirm").find('.btn-yes').text('Yes');
            }
          $('.t6-confirm-action').find('.msg').text(message);
          //Trigger the modal
          $(".t6-confirm-settings").show();
        ...
        initConfirmBox: function(){
            var $message = '<div class="t6-confirm-settings" style="display:none;"><div class="t6-modal-confirm-overlay"></div><div class="t6-modal t6-confirm-action no-title" id="t6-confirm" role="dialog">';
            $message += '<div class="t6-modal-header"><span class="t6-modal-header-title"></span>';
            $message += '<a href="#" class="action-t6-modal-confirm-close"><span class="fal fa-times"></span></a>';
            $message += '</div><div class="t6-modal-content"><p class="msg"></p></div>';
            $message += '<div class="t6-modal-footer"><a href="#" class="btn btn-default btn-no" title="No">No</a><a href="#" class="btn btn-primary btn-yes" title="Yes">Yes</a></div>';
            $message += '</div></div>';
    ...
    $(document).ready(function(){
        ...
        $.fn.rowSortable = function($type){
            $(this).sortable({
                ...
                start: function(event, ui) {
                    var cls = ".t6-"+$type+"-section .row";
                    $(cls).find('.ui-state-highlight').addClass( $(ui.item).attr('class') );
                    $(cls).find('.ui-state-highlight').css( 'height', $(ui.item).outerHeight() );
                },
                stop: function(event, ui){
                    if($type == 'layout'){
                        if(typeof T6Layout !== undefined) T6Layout.layoutApply();
                    }else if($type == 'mega'){
                        if(typeof T6AdminMegamenu !== undefined) T6AdminMegamenu.megaApply();
                    }
                },

            }).disableSelection();
        };
        T6Admin.initSwitchTheme();
        T6Admin.initConfirmBox();

        //update render theme color
        $('.t6-spacer').each(function(e){
            var cls = $(this).attr('class').split(' ');
            var regex = /(.*?)-spacer/gm;
            var groupArr = regex.exec(cls[1]);
            if(groupArr){
                var Sp_cls = groupArr[1] +'-group t6-spacer-heading';
                $(this).closest('.control-group').addClass(Sp_cls);
                $('.'+groupArr[1]+'-color').closest('.control-group').insertAfter($('.'+ groupArr[1] +'-group'));
            }
            
        });
        // $('.user-spacer').closest('.control-group').appendTo($('.t6-spacer').closest(".sub-group-params"));
        // $('.user-spacer').closest('.control-group').addClass("user-group t6-spacer-heading");
        ...
        $('.t6-custom-color-spec').each(function(el){
