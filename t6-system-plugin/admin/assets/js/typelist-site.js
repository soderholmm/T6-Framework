(function($) {
	...
	Site.prototype.get = function () {
		...
		$inputs.each(function(i, el){
			...
			if ($input.hasClass('t6-custom-color-spec') && !value) {
				value = "inherit";
			}
	...
	Site.prototype.set = function (value) {
		...
		$inputs.each( function(i, el) {
			...
			if(name && name.match(/_font_weight/)){
			...
			else if ($input.hasClass('t6-input-color')) {
				var valColor,valName,dataVal;
				if(value[name] != 'none'){
					$input.closest('.t6-select-color').find('li').each(function(){
				...
				if(name == 'megamenu_typo_onoff'){
					if(value[name]){
			            $('.group_styles_font').find('.sub-group-params').find('.control-group.megamenu-setting').not('.t6-checkbox').show();
			        }else {
			            $('.group_styles_font').find('.sub-group-params').find('.control-group.megamenu-setting').not('.t6-checkbox').hide();
			        }
				}
				if(name == 'heading_typo_onoff'){
					if(value[name]){
			            $('.group_styles_font').find('.sub-group-params').find('.control-group.heading-setting').not('.t6-checkbox').show();
			        }else {
			            $('.group_styles_font').find('.sub-group-params').find('.control-group.heading-setting').not('.t6-checkbox').hide();
			        }
				}
			}else if ($input.hasClass('t6-custom-color-spec')) {
			...
			if($input.hasClass('google-font-input')){
				...
				if(value[$typelistName+'_load_weights']){
					var $fontWeightEl = $("<div class='t6-font-weight-preview control-helper' data-fontname='"+value[name]+"' data-loadweights='"+JSON.stringify(value[$typelistName+'_load_weights'].split(','))+"' />");
					...
					if($input.closest('.control-group').find('.t6-font-weight-preview').length){
						$input.closest('.control-group').find('.t6-font-weight-preview').html($fontWeightEl);
					}else{
			...
			if($input.hasClass('t6-input-media') || $input.hasClass('field-media-input')){
				if($input.closest('.field-media-wrapper').find('.add-on.field-media-preview').length){
					...
					if(value[name]){
						var image = $('<img src="'+t6_site_root_url+'/'+value[name]+'" alt="" />');
