jQuery(document).ready(function ($) {
	...
	var Typelist = function ($el) {
		...
		//this.$inputEdit = this.$container.find('input.t6-typelist-edit');
	...
	Typelist.prototype.doload = function (trigger) {
		var url =
			location.pathname +
			"?option=com_ajax&plugin=t6&format=json&t6do=typelist&id=" +
			tempId;
		...
		$.get(url, { task: "load", type: this.type, name: name, t: (new Date).getTime() }).then(function (
			...
			if (response.value) {
				...
				if (response.error) {
					...
					T6Admin.Messages("Your session expired, please login again", "error");
				}
			}
		});
	};
	...
	Typelist.prototype.makeReadonly = function () {
		...
		$(".t6-sidebar-preview, .t6-pn-views")
			.css("pointer-events", "")
	...
	Typelist.prototype.doedit = function ($btn) {
		...
		if(this.$inputEdit.prop('checked') === true){
			...
			if(JSON.stringify(orgvalue) != JSON.stringify(this.script.get())){
				this.$inputEdit.prop('checked',true);
				T6Admin.Confirm(T6Admin.langs['typelistConfirmEdit'+this.type],function(ans){
					if (ans) {
					...
					$.post(url, { task: "delete", type: self.type, name: name }).then(
						function (response) {
							if (response.ok) {
								...
								T6Admin.Messages(T6Admin.langs[$langs + "d"], "success");
								// hide form
								self.makeReadonly();
								self.doload(1);
							} else {
	...
	Typelist.prototype.docancel = function ($btn) {
		...
		if (JSON.stringify(orgvalue) != JSON.stringify(self.script.get())) {
			T6Admin.Confirm(
				T6Admin.langs["typelistConfirmEdit" + self.type],
				function (ans) {
					if (ans) {
	...
	Typelist.prototype.dosave = function ($btn) {
		...
		if (JSON.stringify(orgvalue) != JSON.stringify(value)) {
			...
			var url = location.pathname + "?option=com_ajax&plugin=t6&format=json&t6do=typelist&id=" + tempId;
			var name = this.$input.val();
			var self = this;
			$.post(url, {
				task: "save",
				...
				if (response.ok) {
					...
					T6Admin.Messages(T6Admin.langs.T6TypeListSaved, "message");
				} else {
					if (response.error) {
						T6Admin.Messages(response.error, "error");
					} else {
	...
	Typelist.prototype.dosaveclone = function ($btn) {
		...
		if (!newname) {
			T6Admin.Messages("Enter new name", "error");
			return;
		}
		var self = this;
		$.post(url, {
			...
			if (response.ok) {
				...
				// hide form
				self.makeReadonly();
				T6Admin.Messages(T6Admin.langs.typelistCloneSaved, "message");
			} else {
				if (response.error) {
					T6Admin.Messages(response.error, "error");
				} else {
	...
	DefaultType.prototype.set = function (value) {
		...
		$inputs.each(function (i, el) {
			...
			if ($input.hasClass("t6-input-color")) {
				var valColor = $input
					.closest(".t6-select-color")
