  ...
  $('.btn-action[data-action="custom.addcolor"]').on('click', function() {
    $('.t6-custom-colors-wrap .btn-action.active').removeClass('active');
  ...
  $('.btn-action[data-action="custom.cancel"]').on('click', function() {
    $('.t6-custom-color-wrap .btn-action.active').removeClass('active');
  ...
  var doSave = function() {
    ...
    if (!colors.name) {
      T6Admin.Messages(T6Admin.langs.colorNameEmptyFieldWaring, 'warning');
      return false;
    }
    ...
    if (color) {
      ...
      T6Admin.Messages(T6Admin.langs.colorEmptyFieldWaring, 'warning');
      return false;
    }
    ...
    if (allName.indexOf(nameReplace) == -1) {
      ...
      // T6Admin.initMinicolors($('.control-group.' + nameReplace).find('input.minicolors'));
       $('.t6-color-picker').each(function() {
		    $(this).minicolors();
		    $(this).minicolors({
      ...
      T6Admin.Messages(T6Admin.langs.customColordaplicateWaring, 'warning');
    }
  }
  var doEdit = function(btn) {
    ...
    if (editLabel == null || editLabel == "") {
      T6Admin.Messages(T6Admin.langs.colorNameNoneWarning, 'warning');
  ...
  var updateInputColor = function($data) {
    var $input_color = $(document).find('.t6-input-color[data-val="' + $data.class.replace(/_/g, " ") + '"]');
    if ($input_color.length) {
      $input_color.val($data.name).trigger('change');
    }
  }
  var saveColorFile = function($data) {
    // $data = JSON.parse($('.t6-custom-colors').val());
    var url = location.pathname + '?option=com_ajax&plugin=t6&format=json&t6do=Customcolors&id=' + tempId;
    $.post(url, { task: 'save', data: $data }).then(function(response) {
      if (response.ok) {
        // update status
        if (response.status == 'add') {
          T6Admin.Messages(T6Admin.langs.customColorHasSaved, 'message');
        }
      } else {
  ...
  var updateColors = function(btn) {
    ...
    $(document).find('.t6-input-color').each(function() {
      var $input = $(this),
        $val = $(this).data('val');
  ...
  var doRemove = function(btn) {
    var $btn = $(btn),
      url = location.pathname + '?option=com_ajax&plugin=t6&format=json&t6do=Customcolors&id=' + tempId,
      name = $btn.closest('.control-group').data('class'),
      type = $btn.data('status') ? $btn.data('status') : '';
    if (!name) return;
    T6Admin.Confirm(T6Admin.langs.customColorRemoveConfirm, function(ans) {
      if (ans) {
        $.post(url, { task: 'remove', name: name, type: type }).then(function(response) {
          if (response.ok && response.status) {
            var input_selected_color = $(document).find('.t6-input-color');
            ...
            // update status
            T6Admin.Messages(T6Admin.langs.customColorDeleted, 'message');
          } else {
            T6Admin.Messages(response.error, 'error');
          }
        });
