jQuery(document).ready(function($) {
	...
	$('.btn-action[data-action="addons.addasset"]').on('click', function() {
		$('.t6-addons-wrap .btn-action.active').removeClass('active');
	...
	$('.addons-items').off('click', '.btn-action[data-action="addons.remove"]').on('click', '.btn-action[data-action="addons.remove"]', function() {
		var that = this;
		T6Admin.Confirm(T6Admin.langs.addonRemoveConfirm,function(ans){
		  if (ans) {
	...
	$(document).on('click','.btn-action[data-action="addons.edit"]', function(e) {
	    ...
	    $('.addons-form').find('#addons-name').val(T6Admin.addons[$aname].name);
	    if(T6Admin.addons[$aname].css) $css = T6Admin.addons[$aname].css.join("\n");
	    if(T6Admin.addons[$aname].js) $js = T6Admin.addons[$aname].js.join("\n");
	...
	var doSave = function () {
		var url = location.pathname + '?option=com_ajax&plugin=t6&format=json&t6do=addAddon&id=' + tempId;
		...
		if (!asset.name) {
			T6Admin.Messages(T6Admin.langs.addonEmptyFieldWaring,'error');
			return;
		}
		if((!asset.js && !asset.css)){
			T6Admin.Messages(T6Admin.langs.addonEmptyFieldCssOrJSWaring,'error');
			return;
		}
		if(asset.name != $oldname && T6Admin.addons.hasOwnProperty(asset.name)){
			T6Admin.Messages(T6Admin.langs.addonNameDuplicated,'error');
			return;
		}
			...
			if (response.ok && response.asset) {
				...
				if(response.action == 'update'){
					if($oldname != response.asset.name ){
						...
						delete T6Admin.addons[$oldname];
					}
					T6Admin.Messages(T6Admin.langs.T6AddonsHasUpdated,'message');
					...
					$('.t6-addons-wrap .btn-action.active').removeClass('active');
					T6Admin.Messages(T6Admin.langs.T6AddonsHasAdded,'message');
				}
				$('.btn-action[data-action="addons.addasset"]').show();
				$('li.editting').removeClass('editting');
				T6Admin.addons[response.asset.name] = response.asset;
			} else {
	...
	var doRemove = function (btn) {
		...
		if (!addon){
			T6Admin.Messages('T6_ADDON_NOT_FOUND','error');
			return false;
		}
		var url = location.pathname + '?option=com_ajax&plugin=t6&format=json&t6do=removeAddon&id=' + tempId;
		$.post(url, {name: addon}).then(function(response) {
			if (response.ok) {
				// remove addon
				$btn.closest('li').remove();
				$('.btn-action[data-action="addons.addasset"]').show();
				T6Admin.Messages(T6Admin.langs.addonRemoveDeleted,'message');
			} else {
				T6Admin.Messages(response.error,'error');
			}
		})
