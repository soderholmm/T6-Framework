    ...
    $(document).ready(function() {
        ...
        var validationData = function() {
            if(!$('.t6-pattern.title').val()){
                $('.t6-palette-error').show();
                return false;
            }else{
                $('.t6-palette-error').hide();
                return true;
            }
        }
        ...
        $('body').on('click','.t6-patterns-cancel', function(e) {
            $(this).closest('.t6-palettes-modal').hide();
        ...
        // $(document).find('.config_pattern').find('input.t6-pattern').on('change',function(){
		...
		$('body').on('click','.pt-color-del', function(e) {
            ...
            if(plEl.hasClass('clone')){
                plEl.remove();
                T6Admin.Messages(T6Admin.langs.palettesRemnoveClone,'message');
                return true;
            }
        ...
        $('body').on('click','.t6-pattern, .fa-angle-down', function(e) {
            $('.choose-color-pattern').addClass('hidden');
            $(this).closest('.controls').find('.choose-color-pattern').removeClass('hidden');
        });
        $('body').on('click','.t6-input-color', function(e) {
            $('.choose-color-pattern').addClass('hidden').removeClass('is-focus');
            //init user color to chose color
            var $colorActive = $(this).data('val');
            $(this).closest('.controls').find('.choose-color-pattern').find('.t6-select-pattern[data-val="'+$colorActive.replace(/\s+/g,'_')+'"]').addClass('active');
            $(this).closest('.controls').find('.choose-color-pattern').removeClass('hidden').addClass('is-focus');
        });
         $(document).on('click',function(e){
            if(!$('.t6-pattern-row').is(":hidden")){
                if(($('.fa-angle-down').index(e.target) == -1) && ($('.t6-pattern').index(e.target) == -1) && ($('.choose-color-pattern').index(e.target) == -1)){
                    $('.choose-color-pattern').addClass('hidden');
                }
            }else{
                if($('.choose-color-pattern').hasClass("is-focus")){
                    if(($('.t6-input-color').index(e.target) == -1) && ($('.choose-color-pattern').index(e.target) == -1)){
                        $('.choose-color-pattern').addClass('hidden').removeClass('is-focus');
                    }
                }
            }
        ...
        $('body').on('click','.t6-select-pattern', function(e) {
            var $val = $(this).data('val'),$input = $(this).closest('.controls').find('input.t6-pattern');
            if($input.length == 0) $input = $(this).closest('.controls').find('input.t6-input-color');
