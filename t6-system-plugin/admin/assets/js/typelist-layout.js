(function($) {
	...
	Layout.prototype.set = function (value) {
		...
		$inputs.each( function(i, el) {
			...
			if($input.hasClass('t6-input-media')){
	...
	Layout.prototype.updateLayout = function($data){
		...
		var $layoutBuider = Layout.$container.find('#t6-layout-builder'),
	...
	Layout.prototype.sectionRender = function($section){
		...
		$layout_raw += '<div class="t6-layout-section" '+dataAttr+'>';
		$layout_raw += '<div class="t6-section-settings clearfix">';
		$layout_raw += '<div class="pull-left"><strong class="t6-section-title">'+$section.name+'</strong></div>';
		$layout_raw += '<div class="pull-right"><ul class="t6-row-option-list">';
		$layout_raw += '<li><a class="t6-move-row" href="#" data-tooltip="Move"><i class="fal fa-arrows-alt"></i></a></li>';
		$layout_raw += '<li><a class="t6-row-options" href="#" data-tooltip="Configure"><i class="fal fa-cog fa-fw"></i></a></li>';
		$layout_raw += '<li><a class="t6-remove-row" href="#" data-tooltip="Remove"><i class="fal fa-trash-alt fa-fw"></i></a></li>';
		$layout_raw += '</ul></div></div>';
		$layout_raw += '<div class="t6-row-container"><div class="row">';
      ...
      $layout_raw += '<div class="t6-col t6-layout-col ' + $col_cls + " " + extra_class + '" ' + $attrCol + ">";
      $layout_raw += '<div class="col-inner clearfix">';
      if ($section.contents[i].type == "component") $section.contents[i].name = "Component";
			$layout_raw += '<span class="t6-column-title">' + $section.contents[i].name + "</span>";
			$layout_raw += '<span class="t6-col-remove hidden" title="Remove column" data-content="Remove column"><i class="fal fa-minus"></i> </span>';
			$layout_raw += '<span class="t6-admin-layout-vis" title="Click here to hide this position on current device layout" style="display:none;" data-idx="' + i + '"><i class="fal fa-eye"></i></span>';
			$layout_raw += '<a class="t6-column-options" href="#"><i class="fal fa-cog fa-fw"></i></a>';
			$layout_raw += "</div></div>";
    }
		$layout_raw += '</div></div>';
		$layout_raw += '<a class="t6-add-row" href="#"><i class="fal fa-plus"></i><span>Add Row</span></a></div>';
	...
	Layout.prototype.generatedLayout =  function(){
		var item = [];
		Layout.$container.find('#t6-layout-builder').find('.t6-layout-section').each(function(index){
			var $row 		= $(this),
				rowIndex 	= index,
				rowObj 		= $row.data();
