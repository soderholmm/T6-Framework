...
jQuery(document).ready(function ($) {
	...
	$('.btn-action[data-action="tool.export"]').click(function () {
		var url = location.pathname + '?option=com_ajax&plugin=t6&format=json&t6do=Export&id=' + tempId;
		var groups = '';
		...
		if ($('#tool-export-groups').val()) {
			...
			if (!groups.length) {
				alert(T6Admin.langs.toolExportNoSelectedGroupsWarning);
				return;
			}
			groups = groups.join(',');
		}
	...
	var finishExport = function () {
		T6Admin.Messages(T6Admin.langs.ExportDataSuccessfuly, 'message');
		clearForm('export');
	}
	...
	$('input[name="tool-import-file"]').on('change', function (e) {
		...
		var url = location.pathname + '?option=com_ajax&plugin=t6&format=json&t6do=Import&task=Import&id=' + tempId;
		proccessImportAjax(data, url);
	})

	// modal import bundle 
	var $importModal = $('#t6-tool-import-modal');
	...
	var proccessImportData = function (data) {
		if (!data.dir) {
			clearImport();
			alert(T6Admin.langs.toolImportDataFileError);
			return;
		}
		...
		if (count) {
			...
			// bind action event
			$('.t6-btn[data-action="tool.import"]').off('click').on('click', function () {
				doImport(data);
			})
		} else {
			alert(T6Admin.langs.toolImportDataFileEmptyWarning);
	...
	var doImport = function (data) {
		...
		var groups_data = [];
		var url = location.pathname + '?option=com_ajax&plugin=t6&format=json&t6do=import&task=importing&id=' + tempId;
	...
	var finishImport = function () {
		...
		clearForm('import');
		alert(T6Admin.langs.toolImportDataDone);
		$('.t6-admin-save .btn-save').click();
	}
	...
	$('.t6-btn[data-action="tool.css"]').click(async () => {
		...
		const url = location.pathname + '?option=com_ajax&plugin=t6&format=json&t6do=getcss&id=' + tempId;
	...
	$('body').on('click', '.t6-css-editor-apply', async () => {
		const editor = T6CodeEditor.instances.cssEditor;

		saveCss(editor.getValue());
	});
	var saveCss = function (css) {
		var url = location.pathname + '?option=com_ajax&plugin=t6&format=json&t6do=savecss&id=' + tempId;
		$.post(url, { css: css }).then(function () {
			...
			T6Admin.Messages(T6Admin.langs.customCssSaved, 'message');
		})
	}


	// SCSS TOOLS
	var $scssmodal = $('#t6-tool-scss-modal');
	$scssmodal.appendTo($('.themeConfigModal'));
	$('.t6-btn[data-action="tool.scss"]').click(async () => {
		const scssVariablesEditor = await T6CodeEditor.get({
			name: 'scssVariablesEditor',
			container: '#t6-scss-editor-variables',
			language: 'scss',
		});
		const scssEditor = await T6CodeEditor.get({
			name: 'scssEditor',
			container: '#t6-scss-editor-custom',
			language: 'css',
		});
		...
		const url = location.pathname + '?option=com_ajax&plugin=t6&format=json&t6do=scss&task=load&id=' + tempId;
		const res = await fetch(url);
	...
	$scssmodal.on('click', '.btn[data-action="apply"]', async event => {
		...
		const { scssVariablesEditor, scssEditor } = T6CodeEditor.instances;
		const scssVar = scssVariablesEditor.getValue();
		const scssCustom = scssEditor.getValue();
		const url = location.pathname + '?option=com_ajax&plugin=t6&format=json&t6do=scss&task=save&id=' + tempId;
		...
		try {
			...
			if (result.success || result.ok) {
				...
				T6Admin.Messages(result.message, 'error');
			}
		} catch (error) {
			T6Admin.Messages('Compile error!', 'error');
		}
	...
	$scssmodal.on('click', '.btn[data-action="clean"]', async event => {
		...
		const url = location.pathname + '?option=com_ajax&plugin=t6&format=json&t6do=scss&task=clean&id=' + tempId;
		...
		try {
			...
			if (result.error) {
				T6Admin.Messages(result.error, 'error');
			} else {
				T6Admin.Messages('Remove successfully!');
			}
		} catch (error) {
			T6Admin.Messages(error, 'error');
		}
