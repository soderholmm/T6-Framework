...
$seccls = ['t6-section'];
if (!empty($data['name'])) 
	$seccls[] = ' t6-' . preg_replace('/\s/', '-', strtolower($data['name']));
if(!empty($data['t6-role']))
	$role = ' role="' . strtolower($data['t6-role']).'"';
else
...
if (!empty($data['color_pattern'])) {
  $seccls[] = ' t6-palette-' . $data['color_pattern'];
}
...
$containerClasses = " class=\"t6-section-inner $container\"";
