...
$path = T6PATH_LOCAL . $chromepath;
if (is_dir($path)) {
	$files = Folder::files($path, '\.php$');
	if ($files) $chromes = array_merge($chromes, $files);
}

$path = T6PATH_TPL . $chromepath;
if (is_dir($path)) {
	$files = Folder::files($path, '\.php$');
	if ($files) $chromes = array_merge($chromes, $files);
}

$path = T6PATH_BASE . $chromepath;
if (is_dir($path)) {
	$files = Folder::files($path, '\.php$');
	if ($files) $chromes = array_merge($chromes, $files);
}
...
foreach ($chromes as $chrome) {
	...
	if (!function_exists($func)) {
		eval('function ' . $func . '($module, $params, $attribs) {\T6\Helper\Chromes::render(\'' . $style . '\', $module, $params, $attribs);}');
	}
}
