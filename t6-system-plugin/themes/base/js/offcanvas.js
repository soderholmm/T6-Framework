...
jQuery(document).ready($ => {
	const $offcanvas = $('.t6-offcanvas');

	$('.t6-wrapper').addClass('c-offcanvas-content-wrap');

	if ($('#triggerButton').length) {
		$offcanvas.T6Offcanvas({
			triggerButton: '#triggerButton',
			onOpen: function () {
				$('#triggerButton').addClass('active');
				bodyScrollLock.disableBodyScroll('.t6-off-canvas-body');
			},
			onClose: function () {
				$('#triggerButton').removeClass('active');
				setTimeout(function () {
					bodyScrollLock.enableBodyScroll('.t6-off-canvas-body');
				}, 300);
			}
		});
		setTimeout(() => {
