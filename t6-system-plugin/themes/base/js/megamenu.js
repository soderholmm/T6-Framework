jQuery(document).ready(function ($) {
  ...
  var navitem_selector = ".t6-megamenu .nav-item";
  ...
  var startAnimating = function ($item) {
    // get duration
    var $menu = $item.closest(".t6-megamenu");
  ...
  $(".nav-item.dropdown > .dropdown-toggle").on('touchstart', function (e) {
    ...
    if ($el.parents('.t6-offcanvas').length) {
      return;
    }
    ...
    if (window.outerWidth > 990) {
      ...
      $('.t6-megamenu .nav-item.dropdown').each((i, el) => {
  ...
  $(document).on('click', e => {
    ...
    if ($target.closest('.t6-megamenu').length) {
      return;
    }

    activeNavId = 0;

    $('.t6-megamenu .nav-item.dropdown.show').each((i, el) => {
      hideDropdowns($(el));
    })
  })

  // show toggler
  $(".t6-megamenu").each(function () {
