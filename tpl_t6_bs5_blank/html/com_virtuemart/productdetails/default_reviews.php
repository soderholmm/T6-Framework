 ...
 * @version $Id: default_reviews.php 10649 2022-05-05 14:29:44Z Milbo $
 */
...
?>
    ...
    <form method="post" action="<?php echo Route::_('index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id=' . $this->product->virtuemart_product_id . '&virtuemart_category_id=' . $this->product->virtuemart_category_id, FALSE); ?>" name="reviewForm" id="reviewform">
      ...
      <?php if ($this->allowRating and $review_editable) { ?>
        ...
        <h4><?php echo vmText::_('COM_VIRTUEMART_WRITE_REVIEW');
            ...
            ?><span><?php echo vmText::_('COM_VIRTUEMART_WRITE_FIRST_REVIEW') ?></span>
          <?php } ?>
        </h4>
      ...
      <script>
			jQuery(document).ready($ => {
				jQuery('.rating .ratingbox').mousemove( function(e){
					var steps = <?php echo (int) $maxrating ?>;
...
";
          ...
          <br />
          <textarea class="virtuemart" title="<?php echo vmText::_('COM_VIRTUEMART_WRITE_REVIEW') ?>" class="inputbox" id="comment" onblur="refresh_counter();" onfocus="refresh_counter();" onkeyup="refresh_counter();" name="comment" rows="5" cols="60">
          <?php if (!empty($this->review->comment)) {
      ...
      <?php } else if ($review_editable and $this->allowRating) { ?>
        <input class="highlight-button" type="submit" name="submit_review" title="<?php echo vmText::_('COM_VIRTUEMART_REVIEW_SUBMIT') ?>" value="<?php echo vmText::_('COM_VIRTUEMART_REVIEW_SUBMIT') ?>" />
...
if ($this->showReview) {
  ...
  ?>
  <!-- <h4><?php //echo vmText::_ ('COM_VIRTUEMART_REVIEWS') 
            ?></h4> -->
